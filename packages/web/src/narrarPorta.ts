 ...
 * caminho vivo: o evento `porta` só é emitido quando a carta revelada é monstro.
 * Os casos `raca` e `classe` são DEFENSIVOS, não caminhos vivos: desde a
 * separação porta aberta × porta fechada, a carta que vai para a mão sai pelo
 * evento `achado`, sem carta junto. Eles ficam porque o `never` do `default`
 * exige cobrir a união inteira de `CartaPorta`, e a frase que escrevem
 * (*"encontra uma carta de X"*, sem alarde, contra a exclamação do monstro) é a
 * que valeria no dia em que um caminho vivo existir — não devem virar desculpa
 * para reanexar carta oculta ao evento `porta` antes desse dia chegar.
 *
...
export function narrarPorta(
  ...
  switch (carta.tipo) {
    ...
    case 'classe':
      return `${quem} encontra uma carta de ${nomeDaClasse(carta.classeId)}.`;
    default: {
