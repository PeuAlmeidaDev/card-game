...
| 12 | 🚨 **A decisão #21 CONTRADIZ o §4 deste documento, e a contradição é load-bearing.** #21 afirma que "a família Tesouros é **equipamento-only por desenho**", justificando com *"§4 e §6.2 já diziam"*. Mas **§4 (linha 94) e §11 listam QUATRO tipos** — equipamento, instantâneo, item de batalha, item que atrapalha batalha — e **não existe §6.2** neste documento. ⚠️ Não é cosmético: `fase.ts:81` usa `mochila.length > 0` em vez de `.some(c => c.tipo === 'equipamento')`, com comentário justificando pela #21; se os outros três tipos existirem, essa linha fica **silenciosamente errada** (o auto-pulo acha que há o que equipar quando só há um instantâneo guardado). E "item de batalha" é **metade da mecânica de interferência** — decidir isto decide o escopo daquela fatia. **Achado em 2026-07-28, ao montar o roteiro para o MVP.** | §4, §11, §19 |
