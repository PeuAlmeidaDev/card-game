...
| 2 | 🚧 **Maldições / Bad Stuff — EM CURSO, metade construída**, e 🧩 **DECOMPOSTA EM QUATRO FATIAS em 2026-08-08 (decisão #110)** | ~~**2a** Bad Stuff + evacuação~~ ✅ **2026-08-09** → ~~**2b** consumíveis (`instantâneo`)~~ ✅ **2026-08-09/10** → ⏭️ **2c** maldição no `vasculhar` → **2d** maldição na mão (mira + concorrência), com a **2a-bis** (pilhagem do cadáver, #117) **candidata a trocar de lugar** com as duas. 📄 **Specs escritos:** `2026-08-08-bad-stuff-e-evacuacao-design.md` e `2026-08-09-consumiveis-instantaneo-design.md`; **a 2c e a 2d não têm spec.** ✅ **E o "conserto da economia" desta linha está MEDIDO:** o esgotamento do baralho de Itens foi de 90,83% a **0%** das partidas (#135), 🔴 com a atribuição **sobredeterminada** entre tamanho e proporção (#136) | É a **1ª carta que mira outro jogador** e o **conserto da economia** — sem ela o baralho de Itens seca em 20/20 partidas (#46). ⚠️ **Adiado por três fatias** pela #61 — a `afinidade` **alivia** o mesmo sintoma (50% mais carta de Item), mas alívio não é conserto, e as três fatias acabaram. 🔴 **Como estava escrito aqui, o bloco era SEIS eixos independentes**, e os dois caminhos de volta de carta (evacuação e consumíveis) entrariam juntos — nenhum número diria qual consertou a economia (#110). ⚠️ **O 2d está BLOQUEADO pela pergunta 16 do §18**, que segue aberta |
...
| 5 | **Interferência** | janelas A/B, contratos no server, **motor para N** (#33), `carta de combate`. ✏️ **CORREÇÃO MARCADA (2026-08-10): o `instantâneo` SAIU desta célula — ele foi construído no bloco 2b e NÃO depende da interferência** (#44). Era a mesma contradição da tabela do §17, e foi ela que sustentou a proposta de pular a fatia 2b | §12: *"requisito estrutural do MVP, **não item adiável**"*. É a mecânica anti-tempo-morto |
...
| Cortado | Por quê |
|---|---|
| **Habilidade ATIVA de classe** (decisão #49) | Classe entra como carta com **modificadores + passiva** — ✅ **construído em 2026-08-08** (#88, #91, #92); a máquina de habilidade ativa não existe. E a razão principal dela — *"dar decisão ao combate"* — foi resolvida pela #44, o instantâneo nas pausas do motor, ✅ **que deixou de ser promessa e virou código em 2026-08-10** (#127). ⚠️ **A #49 segue intacta:** a fatia `classe como carta` entregou a metade que o bloco 6 do §17 prometia com passiva, e **não** antecipou a ativa |
...
- **Itens** (≈ Tesouros): o loot. **Três** tipos, separados por **quando são jogados**
  ...
  ✅ **O `instantâneo` EXISTE EM CÓDIGO desde 2026-08-10** (fatia `consumíveis (instantâneo)`, a
  **2b**, decisões **#127–#140**): **4 cartas**, 1 cópia por jogador, **16 na mesa de 4**, jogáveis
  da **mão ou da mochila** na fase `combate`. 🔑 **E o ALVO mora na AÇÃO, não na carta** (#128) — é
  isso que dá a ele **exatamente a assinatura da `carta de combate`** desta tabela: no bloco 5 o que
  muda é *quem pode jogar*, não a carta nem o interpretador.
...
✏️ **CORREÇÃO MARCADA (2026-08-10) — um terço desse "nada" saiu.** O **`instantâneo` está
construído** (#127); continuam sem existir em código o **modificador de monstro** e a **carta de
combate**, e o baralho de Itens **deixou de ser equipamento-only**: são **12 equipamentos + 4
instantâneos por jogador**. ⚠️ **O parágrafo acima fica intacto de propósito** — ele registra o que
era verdade em 2026-07-29, e é ESTA linha que diz o presente.
🔑 **A #29 escreveu, em 2026-07-29, o dia da própria morte de um comentário:** *"`fase.ts` está
**certo hoje e errado no dia em que o primeiro instantâneo existir**"*. Esse dia foi **2026-08-09**,
e o comentário morreu na Task 2 desta fatia — **a previsão estava certa e o texto foi trocado antes
de mentir**. É o único caso nesta base em que o vício nº 1 foi **agendado por escrito** e cumprido no
prazo.
...
- **1 raça** + **1 classe** (padrão).
  - Raça = **uma passiva, não stats** (corrigido 2026-07-24 — ver `docs/game-design/mecanica-cartas.md` §5). Stats vêm dos **itens**. Isso mantém o **Humano** (sem carta de raça) jogável: jogar uma raça é **trocar generalismo por especialização**, não ganhar poder bruto de graça.
    - 🔴 **ESTA LINHA ESTÁ REVOGADA POR DECISÃO E NÃO POR CONSTRUÇÃO (decisão #107, 2026-08-08):** a raça passa a declarar um **TETO** de habilidade e de agilidade, somado ao da classe. **Teto é stat, ainda que negativo.** ⚠️ **A linha NÃO foi reescrita de propósito** — ela descreve o **presente**, e o presente ainda é *"raça = só passiva"* (`packages/cartas/src/racas.ts` não tem um único número). Reescrevê-la antes de construir seria o vício nº 1 deste projeto com o sinal invertido. ⬜ **E a justificativa dela — manter o Humano jogável — precisa ser REEXAMINADA, não contornada:** sob a #107 o Humano passa a ser *"o teto-base, sem inclinação"*, o que pode ser uma identidade boa (generalista) ou um assento estritamente pior. **Pergunta 21 do §18.**
  ...
  - 🎓 ✅ **Quem está sem carta de classe em jogo é o APRENDIZ** (decisão #60, **construído em
    ...
    **6,51%** dos assentos terminam a partida sem classe, e em 3 de cada 4 partidas ninguém termina
    ...
    especializa.
- **5 encaixes de equipamento em QUATRO famílias:** **Capacete · Armadura · Mão · Pés** — e a
  família **Mão** tem **DUAS VAGAS EQUIVALENTES**. O corpo continua com cinco encaixes físicos
  (`maoDireita` e `maoEsquerda` no código); o que as duas mãos **não** têm mais é diferença de
  regra.
  - 🖐️ ✅ **CONSTRUÍDO em 2026-08-08 (fatia `empunhadura dupla`, decisões #98–#104): todo item de
    uma mão — arma **e** escudo — declara `'mao'` e cai em qualquer vaga livre** (#98). Empunhar
    **duas armas** é jogada normal, e **dois escudos** também é legal — consequência **aceita**,
    não esquecimento.
    ⚠️ **Até 2026-08-08 esta linha listava *"Mão direita · Mão esquerda"* como slots distintos, e o
    catálogo era FIEL a ela:** as **três** armas do jogo declaravam `maoDireita` e a mão esquerda
    tinha **exatamente uma** opção no jogo inteiro (o Escudo Redondo). Não era bug de código — era
    **o modelo**, e a queixa da **#39** já previa o mecanismo. Quem ler *"5 slots"* numa citação
    antiga está lendo o desenho de antes desta data.
  - Arma de **duas mãos** ocupa **as duas vagas**, com a **mesma instância** nos dois encaixes.
  - 🎯 **Com as DUAS vagas ocupadas, quem escolhe qual liberar é o JOGADOR, na própria ação** (#99)
    — não há pendência nova. Com **ao menos uma vaga livre** não há escolha a oferecer: o item
    entra na vaga livre.
...
2026-08-02): **12 itens** cobrindo as 4 famílias (`packages/cartas/src/itens.ts`) — **8 comuns + 4
exclusivos, um por raça sacável** (`orc`, `anao`, `elfo`, `aquatico`; ⚠️ a conta sai de
`RACAS_SACAVEIS`, que **exclui o Humano** — ver #54). **Nenhum item entrou nem saiu na
`empunhadura dupla`** — só a declaração de `slot` de quatro deles mudou (#98), e por isso o §11
**não muda**. A repartição de hoje é **capacete 3 · armadura 3 · mão 4 · pés 2**. O Montante
**segue sendo a única arma de duas mãos do catálogo de produção** — nenhum dos quatro exclusivos é
—, e ocupa as duas vagas de mão com a **mesma instância**. `combatenteDe(jogador, catalogo)`
...
contrajogo. Encaixe nomeado é onde nasce a escolha interessante, e é a âncora natural das skins.

⚠️ **O exemplo que esta linha usava MORREU em 2026-08-08, e não é troca de redação:** ela escrevia
o trade-off como *"essa espada é melhor, mas é de duas mãos e **eu perco o escudo**"*, o que só faz
sentido enquanto a mão esquerda for a casa do escudo. Com as duas mãos como vagas equivalentes
(#98), o custo do Montante **não é perder o escudo** — é **abrir mão de DUAS armas de uma mão**.
🔴 **E esse trade-off está hoje DESEQUILIBRADO, de propósito e medido:** duas Espadas Curtas dão a
mesma força **+4** sem o **−1** de agilidade do Montante, então ele fica **estritamente dominado**
(#100). O trade-off que o jogo oferece de verdade passa a ser ***"uma arma grande ou duas
pequenas?"***, e ele **só volta a ser escolha quando o dial girar** — 🎚️ adiado para uma passada
própria, com número na mão (pergunta **20** do §18).
...
- Perder combate / cartas ruins → **Bad Stuff** (efeito da carta). ✅ **CONFIRMADO POR MONSTRO em
  2026-08-08** (decisão **#112**): o Bad Stuff é dado da **carta**, não regra uniforme da derrota.
  - ✅ **E ele é INCONDICIONAL — na DERROTA** (decisão **#118**, 2026-08-09): *"você sempre toma a
    badStuff a não ser que ele tenha alguma carta que diga o contrário"*, e o Pedro **qualificou o
    "sempre" explicitamente**: *"deixando claro, é caso ele seja **derrotado**"*.
    🔴 **A qualificação importa porque esta linha do §10 tem DUAS fontes de Bad Stuff** —
    *"perder combate / **cartas ruins**"*. O "sempre" da #118 cobre **só a primeira**. A rota das
    **cartas ruins** (maldição) é a **2c/2d**, tem mira e concorrência, e **nada aqui diz que ela é
    incondicional**. ⚠️ Ler a #118 como *"maldição também não tem defesa"* seria estender uma
    decisão para além do que ela decidiu.
    🔑 **A escapatória é CONTEÚDO, não ausência** — vem de uma carta que o jogador **joga**, no eixo
    dos consumíveis (**2b** / #40), e **nunca** de um monstro que não pune. ⬜ **Essa carta não
    existe** e é fatia futura.
  - 🖥️ **O Bad Stuff é PÚBLICO e fica escrito na carta do monstro** (decisão **#119**, 2026-08-09):
    o jogador lê o que aquele monstro faz com ele **antes** de escolher a luta. É coerente com a
    carta de monstro ser revelada **com a face para cima** e com os stats dela já serem informação
    pública.
  - 🎲 **Um monstro pode ter MAIS DE UM efeito** (decisão **#120**, 2026-08-09) — hoje todos têm
    exatamente um; a lista existe para os designs futuros.
  - 🎚️ **O vocabulário de hoje são dois verbos** (#114): `evacuacao` e `perdeSlot(família)`, e **só
    o monstro de 3 tesouros — o Ogro — evacua**, 1 de 5. ✅ **CONSTRUÍDO em 2026-08-09.** A
    atribuição em produção: **Rato `pes` · Goblin `capacete` · Lobo Sombrio `mao` · Carniçal
    `armadura` · Ogro `evacuacao`**. 🔑 **`perdeSlot('mao')` limpa OS DOIS encaixes e deduplica por
    id** — o Montante é uma carta em duas vagas, e limpar uma só o deixaria vivo dando stats cheios.
    🔴 **O item arrancado vai DIRETO ao cemitério de Tesouros**, nunca à mochila: assimetria
    deliberada com `destinoDoDesequipado`, porque trocar equipamento é **sua escolha** e o Bad Stuff
    é o monstro **tomando**. ✅ Consequência de graça: `perdeSlot` **nunca abre pendência de queima**.
    📊 **Medido: 32,6%–33,3% dos `perdeSlot` acertam um encaixe JÁ VAZIO** (#124) — o *"escapa de
    graça"* é **1 em 3**, não exceção, e o evento é emitido **mesmo assim** (é a #28 valendo).
- **"Morte" = evacuação:** perde **todas as cartas** (mão, equipamentos, mochila) e **mantém a
  patente**. Recomeça comprando mão nova. Continua no jogo. ✅ **CONSTRUÍDO em 2026-08-09**, medido em
  **0,364 evacuações por jogador por partida** (#124).
  - ✅ **"MANTÉM A PATENTE" CONTINUA VALENDO, e sobreviveu a uma tentativa de reversão no mesmo
    dia** (decisões **#111 → #113**, 2026-08-08). O Pedro decidiu resetar a patente, a objeção
    escrita do parágrafo *"Por que mantém a patente"* logo abaixo foi apresentada a ele, e ele
    **reverteu na mesma sessão**: *"não precisa resetar a patente então"*. 🔑 **O que a #111 deixou
    de útil e FICA:** o *"recomeça comprando mão nova"* desta linha ganhou **número** (**4 Portas +
    4 Tesouros**) e **momento** (o **turno seguinte do próprio jogador**, não imediatamente) — as
    duas eram lacunas reais deste §10.
  - 🔁 **O RECOMEÇO, como ele ficou em código** (#116, e as **#121/#122** que a execução acrescentou):
    quem evacuou compra **4 Portas + 4 Tesouros** no **turno seguinte dele** e entra em **`recompor`
    CRAVADO**, nunca em `faseDoTurnoDe`. 🔴 **Ele volta ACIMA do teto de propósito** — com raça o
    limite é 7 e `4+4 = 8` —, porque em `recompor` há três saídas (equipar, guardar, jogar
    raça/classe) e só a `descartar` teria a caridade como única saída.
    🔴 **E a compra ANEXA à mão, nunca substitui** (#121): quem evacua mantém a patente, logo
    continua alvo **legítimo** de caridade enquanto espera a vez — a carta recebida nesse intervalo
    **fica**, e por isso ele pode voltar com **mais** de 8. 🔴 **Se o baralho de Tesouros estiver
    seco, ele leva o que houver** e o jogo emite `tesouroEsgotado` (#122); **Portas não tem esse
    guard**, pela #62.
  - ✅ **DECIDIDO em 2026-08-08 (#115): a evacuação NÃO leva `emJogo.raca` nem `emJogo.classe`.**
    Leva mão + mochila + os cinco encaixes; ficam a patente (#113), a especialização e o contador de
    derrotas. ✏️ *Esta linha ficou marcada como **⬜ não decidido** até 2026-08-09, com a #115 já
    escrita no §19 desde o dia anterior — a decisão foi registrada e a seção temática não foi
    atualizada junto, que é o passo 2 da regra do documento vivo.* 🔑 **A razão mais forte é a #38:**
    a carta de raça é *"artefato de transformação, consumido no uso"* — o pergaminho já foi gasto, e
    mandá-lo ao cemitério seria **desconsumir** algo consumido.

✅ **TUDO ISTO EXISTE EM CÓDIGO desde 2026-08-09** (fatia **2a**, `Bad Stuff e evacuação`).
✏️ **CORREÇÃO MARCADA:** este parágrafo dizia *"🔴 **NADA DISTO EXISTE EM CÓDIGO** (2026-07-29).
Perder um combate hoje só incrementa um contador"*, e deixou de ser verdade quando a 2a foi
construída. O que existe hoje, dentro do `fecharCombate` (âncora, **não** linha — ela já andou duas
vezes):

- `MonstroCarta.badStuff`, **obrigatório e lista** (#114/#120), lido pelo reducer via
  `InfoMonstro.badStuff` e interpretado pela função **pura** `aplicarBadStuff`
  (`packages/partida/src/badStuff.ts`), `switch` fechado por `never`.
- A **evacuação** (#115) e o **recomeço** em `recompor` com 4+4 (#116, #121, #122).
- Os dois eventos narrados (`perdeuEquipamento`, `evacuou`) e o texto **na carta do monstro** (#119).

🎯 **A razão de a fatia existir, e o que a medição respondeu:** *"a evacuação é o maior caminho de
VOLTA de cartas do jogo inteiro"* (#46) — devolve mão, corpo e mochila ao cemitério de uma vez. Ela
foi medida em **+13,57 cartas por partida** sobre uma punição leve (#123).
🔴 **E ela NÃO consertou a economia sozinha:** o baralho de Tesouros ainda esgota em **91,7% das
partidas** (#125), como o spec previu por escrito. A **#40** segue sendo a resposta **estrutural**, e
os consumíveis do bloco **2b** seguem com trabalho.

**Por que mantém a patente** (correção de uma versão anterior deste doc, que mandava pro nível 1)
— 🔑 **e este parágrafo já se defendeu DUAS vezes: ele nasceu corrigindo uma versão que mandava
pro nível 1, e em 2026-08-08 derrubou a decisão #111, que tentava voltar para lá (ver #113):**
...
- **Três** tipos de item, separados por **quando são jogados** — tabela completa no §4
  ...
  **instantâneo** (durante o combate, só o lutador — #44). ✅ **DOIS dos três existem em código desde
  2026-08-10**: equipamento (fatia 8) e **instantâneo** (fatia `consumíveis (instantâneo)`, #127).
  ⬜ **A `carta de combate` continua sem existir** — ela é do bloco 5, e a **#21 não a cancelou**
  (decisão #29).
...
- 🔴 **A caridade de TESOURO NÃO voltou com o corte da `salaVazia`: zero em 80 partidas**
  ...
  ✏️ **CORREÇÃO MARCADA (2026-08-10): a PREMISSA deste bullet morreu e a conclusão NÃO foi
  remedida.** `CartaTesouro` tem **dois** variantes desde a fatia `consumíveis (instantâneo)`, e o
  baralho de Itens **não é mais 100% equipamento**. 🔴 **Mas isso não ressuscita a caridade por si:**
  o bot foi ensinado a **guardar** o instantâneo na mochila, então ele também não sobrevive até
  `descartar` — muda o **mecanismo**, não o resultado esperado. ⚠️ **O soak da 2b NÃO instrumentou
  caridade**, então isto é **lido no código, não medido**. Quem for remedir lê a política do bot de
  hoje, nunca esta frase.
...
- 🔴 **O PRIMEIRO CAMINHO DE VOLTA EXISTE DESDE 2026-08-09 — e ele NÃO bastou.** A evacuação do §10
  devolve mão + mochila + os cinco encaixes aos cemitérios de uma vez, e ela foi **medida com
  controle interno** (mesmo build, mesma sessão, uma variável — o `badStuff` do Ogro):
  **+13,57 cartas por partida** sobre uma punição leve (#123). 📊 **E o baralho de Tesouros continua
  esgotando em 91,7% das partidas** (contra 97,9% no braço de controle; **N=240 partidas por
  braço**, #125). ⚠️ **A queda NÃO tem causa atribuída** — o braço de controle se moveu **+1,2pp
  entre duas rodadas com o MESMO código**, então o movimento do outro braço cabe no ruído de sessão.
  ➡️ **Isto é evidência A FAVOR da #40, exatamente como o "dobrar o baralho" foi**: o que trava a
  carta não é o tamanho do monte nem a falta de um caminho de volta pontual — é ela **nunca
  circular**. **Nenhum consumível existe em código**, e eles nascem no bloco **2b**.
  🔑 **A #114 já tinha escrito o teste desta hipótese:** *"se a evacuação sozinha consertasse a
  economia, o 2b ficaria sem trabalho"*. **Ela não consertou. O 2b tem trabalho.**
- ✅ **O SEGUNDO CAMINHO DE VOLTA EXISTE DESDE 2026-08-10, e ele MOVE — a carta que circula.** Com a
  mesa de produção de hoje (64 Tesouros, 25% consumível) o baralho **não esgotou em nenhuma das 240
  partidas** medidas, contra **90,83%** do controle de 48 só-equipamento (#135). 🔴 **E o mérito
  disso NÃO é atribuível a uma alavanca só:** um baralho de **64 puramente de equipamento** apaga o
  esgotamento igualmente bem (**braço C**), e trocar 4 equipamentos por 4 consumíveis **dentro do
  baralho de 48** derruba o esgotamento de **86,25% para 2,08%** (**braço D**, à dose de **33,3%**,
  #136). ➡️ **O efeito é SOBREDETERMINADO: tamanho e proporção bastam, cada um sozinho.**
  🔑 **A leitura que fecha as duas, e é ela que serve para desenhar baralho: a PROPORÇÃO move o
  DEGRAU, o TAMANHO compra a MARGEM.** A produção de hoje puxou as duas ao mesmo tempo, e isso não é
  redundância — é margem.
  ⚠️ **A dose de produção (25%) num baralho de 48 NÃO foi medida por nenhum braço.** Quem citar o
  `86,25% → 2,08%` sem a dose autoriza, sem querer, uma decisão de baralho que ninguém mediu.
  📊 **E a #40 continua com metade do alvo por construir:** os `≥ ~50%` dela são `instantâneo` +
  `carta de combate`, e a segunda é do bloco 5.
- 🎯 **A chave da economia de Itens** (diagnóstico aceito em 2026-07-29): **o equipamento é o único
  ...
  ✅ **MEDIDO em 2026-08-10, e o diagnóstico deste bullet se confirma pela propriedade que ele
  nomeia:** um consumível termina a partida **preso** com um jogador em **35,44%** das
  carta × partida, contra **85,56%** do equipamento — **2,41× menos aderente, no MESMO braço, na
  MESMA sessão** (braço B, sessão 3; N = 16×240 e 48×240). É esta a propriedade que a #40 comprou.
  🔴 **E ela NÃO é constante:** a aderência **sobe quando a oferta da família encolhe** (#137) — o
  mesmo instantâneo vai a **59,27%** num baralho de 48. ➡️ **Dobrar a dose não dobra a circulação.**
  🔴 **MAS o *"aumentar o baralho trata o sintoma"* deste bullet ficou COM FRONTEIRA, e ela tem que
  ser escrita:** de **32 → 48** o tamanho moveu só o *quando* (#75); de **48 → 64** ele moveu o *se*
  — o braço C, **64 cartas sem um único consumível**, esgotou **0/240** (#135). ➡️ **A frase valia no
  intervalo em que foi medida e não vale fora dele.** O que sustenta a #40 hoje **não** é *"tamanho
  não move"* — é que a proporção move **dentro do tamanho velho** (#136) e é a única alavanca que
  também compra **variedade e decisão de combate**. ⚠️ **ONDE o degrau cai não foi medido:** nenhum
  braço variou o tamanho com a composição fixa.
- ✅ **Composição de Tesouros em produção — RECEITA DECLARADA desde 2026-08-10** (decisão **#133**):
  **12 equipamentos + 4 instantâneos por jogador = 16/jogador, 64 na mesa de 4**, com **25% de
  consumível**. 🔑 **A desculpa de derivar do catálogo morreu exatamente onde ela mesma marcou:** a
  redação anterior desta linha (#75, 2026-08-02) dizia *"no dia em que o primeiro consumível nascer,
  esta linha vira receita declarada, e é a **#40** cobrando"* — foi o que aconteceu, na borda
  (`server/src/app.ts`), com a proporção assinada em vez de acidental.
  ⚠️ **Composição ANTERIOR, para quem citar medição velha:** 1 carta por item do catálogo, por
  jogador = **12/jogador, 48 na mesa** (#75; e antes disso 8 / **32**). Todo número de soak medido
  até 2026-08-09 inclusive — o `480/480` da `afinidade`, os `97,9%`/`91,7%` da **2a** — sai dessa
  mesa de 48, **não** da de hoje.
...
- ✅ **RECEITA-ALVO DO MVP: 168 cartas na mesa de 4** (decisão #41) — ancorada no Munchkin base,
  que tem 168 (95 Portas + 73 Tesouros). **Hoje temos 132: 68 Portas + 64 Tesouros** (recontado a
  partir do código em 2026-08-10, na fatia `consumíveis (instantâneo)`; era **116** = 68 + 48 até
  ela, e 104 = 56 + 48 até a fatia `classe como carta`). 🎚️ Dial, e **ainda não aplicável**: cita
  três famílias que não existem em código. Cada fatia que construir uma família move a receita real
  na direção dela.
  ...
  do código, sempre.**
  🔴 **O descompasso que a fatia `afinidade` criou dentro desta tabela, porque ele é o argumento da
  #40 em números:** a receita-alvo pede **9 cartas de equipamento por jogador** e o catálogo de hoje
  produz **12** — o equipamento está em **133% do alvo dele**. ✏️ **CORREÇÃO MARCADA (2026-08-10):**
  a frase seguia *"enquanto carta de combate (5) e instantâneo (4) estão em ZERO"*, e **metade disso
  morreu** — o **instantâneo está em 4/jogador, exatamente o alvo dele** (#133). Quem segue em
  **ZERO** é só a **carta de combate**, e é ela que separa os **25%** de consumível de hoje dos
  **≥50%** da #40.
  ...
  | Modificadores de monstro | 3 | **Instantâneo** | **4** ✅ **ATINGIDA em 2026-08-10** (#133) |
  | Pergaminhos de raça | 3 | **consumível** | **50%** — hoje **25%** |
  | **Classes** | **3** ✅ **ATINGIDA em 2026-08-08** (#93) | | |

  ✅ **O instantâneo é a SEGUNDA família a bater o número dela** (depois da classe), e a primeira do
  lado dos Itens. ⚠️ **Bater o alvo por família NÃO é bater a proporção:** os 25% de hoje saem de
  `4 instantâneos ÷ 16 cartas`, e só chegam a 50% quando a `carta de combate` nascer — o equipamento
  em 133% do alvo empurra a fração para baixo pelos **dois** lados.
...
| — | ✅ **`classe como carta`** (decisão #60) — **CONSTRUÍDA em dois planos: A em 2026-08-06 (#87) e B em 2026-08-08 (#88–#97)** | a classe entra pelo baralho (17/jogador, 68 na mesa), o **Aprendiz** é a ausência com `+1` de mochila, as três classes ganham **passiva de combate**, e morreram `classeId`, o `escolhasSchema`, a rota `/duelo` e o construtor no `web`. ✅ **Gate ocular fechado pelo Pedro em 2026-08-08** — os **7** itens do roteiro rodados e aprovados; **MERGEADA** no PR #34. 🔑 **O gate achou um bug que 661 testes e a revisão ampla do branch não pegaram** (a 3ª vez seguida nesta base): duas armas de uma mão nunca coexistem, porque as três armas do catálogo declaram `maoDireita`. **Pré-existente desde o Plano 3a e ortogonal** — virou a fatia `empunhadura dupla`, abaixo | ✅ |
| — | ✅ **`empunhadura dupla`** — **CONSTRUÍDA em 2026-08-08** (spec `2026-08-08-empunhadura-dupla-design.md`, plano `2026-08-08-empunhadura-dupla.md`; decisões **#98–#104**) | a arma de uma mão passa a caber em **qualquer** mão — as duas viram **vagas equivalentes**, e todo item de uma mão declara `'mao'` —, e o jogador escolhe qual liberar **na própria ação** (sem 4ª pendência). Fecha a queixa da **#39** sobre as mãos: a família passa de **3+1 para 4 itens numa vaga dupla**, ⚠️ e a queixa **continua de pé para `pes`**, que tem 2. **Nenhuma carta entrou ou saiu** — só a declaração de `slot` de quatro itens, e por isso o §11 não muda. 🎚️ **O Montante ficou DOMINADO e o dial NÃO girou** — uma variável por vez (#24/#25/#51/#69); virou a pergunta **20** do §18. 🔴 **Nasceu do gate ocular do Pedro**, não de um roteiro planejado. ✅ **Gate ocular RODADO em 2026-08-08** — ele jogou e reportou *"aparentemente tudo ok"*, autorizando o merge; ⚠️ **é conferência em partida real, NÃO o roteiro de 5 itens um a um** — os itens **4** e **5** são cenário forçado (o Montante sobre duas armas, e a volta) e ficaram **sem relato** | ✅ |
| 2 | 🧩 **Maldições / Bad Stuff** — **DECOMPOSTO EM QUATRO pela #110:** `2a` Bad Stuff + evacuação → `2b` consumíveis (`instantâneo`) → `2c` maldição no `vasculhar` → `2d` maldição na mão (mira + concorrência) | a 1ª carta que **mira outro jogador**, + a **morte/evacuação** do §10 — que é o **conserto da economia** (#46). 📌 É ele que finalmente encara a **pergunta 11**. 🔴 **Por que decompor:** como o §3.1 escrevia, eram **seis eixos**, e os **dois caminhos de volta de carta** entrariam juntos — **nenhum número diria qual consertou**. É a #51 outra vez | ✅ |
| 2a | ✅ **`Bad Stuff e evacuação`** — **CONSTRUÍDA em 2026-08-09** (spec `2026-08-08-bad-stuff-e-evacuacao-design.md`; a forma nas **#112–#120**, os números medidos nas **#121–#126**) | perder um combate passa a **custar**: `MonstroCarta.badStuff` obrigatório e lista, o interpretador puro `aplicarBadStuff`, a **evacuação**, o **recomeço** com 4+4 em `recompor`, dois eventos narrados e o texto **na carta do monstro**. 📊 **Margem medida: +13,57 cartas/partida** aos cemitérios (#123). 🔴 **NÃO consertou a economia sozinha** — 91,7% das partidas ainda esgotam (#125), **como o spec previu**. 🔴 **O soak achou DOIS bugs reais** que 730 testes e as revisões das OITO tasks de código anteriores não pegaram, os dois em `comprarMaoInicial` (#121, #122). 🔴 **Gate ocular PENDENTE** | ✅ |
| 2a-bis | ⬜ **Pilhagem do cadáver** (#117) — fatia própria, **desenho NÃO fechado** | os outros três escolhem 1 carta cada dos despojos do morto. Tirada da 2a pelo **tamanho**: zona pública nova + verbo + **pendência que atravessa turnos** + a 1ª quebra de sigilo. ➡️ **Candidata a trocar de lugar com a 2c/2d** na ordem | ✅ |
| 2b | ✅ **`Consumíveis (instantâneo)`** — **CONSTRUÍDA em 2026-08-09/10** (spec `2026-08-09-consumiveis-instantaneo-design.md`; a forma nas **#127–#134**, os números medidos nas **#135–#140**) | a **quarta família de Itens**: 4 cartas de delta de stats, **alvo escolhido na ação**, jogáveis da **mão ou da mochila** na fase `combate`, consumidas ao cemitério — e a receita de Tesouros vira **declarada** (16/jogador, 64 na mesa, **25% consumível**). 📊 **O esgotamento do baralho de Tesouros foi a ZERO em 240 partidas** (contra 90,83% do controle de 48), 🔴 **e a atribuição é SOBREDETERMINADA**: um quarto braço mostrou que a proporção **sozinha**, no baralho velho, também basta (86,25% → 2,08%, à dose de 33,3% — #136). ✅ **3.260 usos medidos**, 100% das partidas com ao menos um, **75,6% saindo da mochila** (#138). 🔴 **Gate ocular PENDENTE** | ✅ |
...
| 5 | **Interferência** | janelas A/B + contratos no server + **motor para N** (#33) + a `carta de combate`. ✏️ **CORREÇÃO MARCADA (2026-08-10): esta linha listava também o `instantâneo`, e era a linha VELHA.** A **#110** (2026-08-08) já o tinha antecipado para a `2b`, e as duas linhas conviveram contradizendo-se por dois dias. 🔴 **Foi ESTA célula que sustentou a proposta de pular a fatia 2b** (*"os consumíveis só vão valer na fase de atrapalhar"*) — o §1.1 do spec registra o episódio. **O `instantâneo` está CONSTRUÍDO desde 2026-08-10 e NÃO depende da interferência** (#44: custo de ritmo zero, com o motor já parando duas vezes por round). O que sobra aqui é a `carta de combate`, que depende da **janela A** | ✅ §12: *"requisito estrutural, não item adiável"* |
...
⚠️ **As fatias sem NÚMERO (`—`) são de propósito** — mesmo motivo
...
carta`, `empunhadura dupla`), que é a regra geral da #48. Elas rodam **nesta ordem**, entre o bloco
1 e o bloco 2. ⚠️ **As três primeiras nasceram juntas em 2026-07-31 (#61); a `empunhadura dupla`
NÃO** — ela entrou em 2026-08-08, vinda do **gate ocular** da fatia anterior, e é o único item desta
lista que nenhuma sessão de design planejou.
...
✅ **Fechada em 2026-08-09 (fatia `consumíveis (instantâneo)`): a 15** — *"um `instantâneo` que dá
+vida CURA ou só levanta o teto?"*. **Resposta do Pedro: cura, com TETO na vida inicial do alvo**
(decisão **#129**). ⚠️ **A pergunta estava escrita num vocabulário que o motor não tem** — não existe
"vida máxima" em `Combatente`, `vida` é um contador que só desce. A tradução única em código é
`min(vida + n, vidaInicial)`, e é ela que faz jogar a poção com a vida cheia **desperdiçar** — que é
a decisão *"agora, ou aguento mais um golpe?"*. 💰 **Custo aceito e declarado antes de medir:** a
carta com teto **circula menos** que a sem teto. 📊 Ela ficou em **~43%** dos usos das outras três
(braço B), e o soak mostra que a causa é **política do bot** (janela própria, só abaixo de 40% da
vida), não a regra.
...
| 17 | 🔴 **A ORDEM DO ASSENTO decide a partida hoje, e ninguém decidiu isso.** Medido em 2026-08-01 com os **quatro assentos rodando exatamente a mesma política**: **#0 vence 40,6% · #1 26,6% · #2 20,0% · #3 12,8%** (n=320; χ² = 53,78 com df=3, p < 10⁻¹⁰; tendência z = −7,20; #0 × #3 z = 7,36). Num jogo **competitivo ranqueado** (§1, §3), *"quem senta primeiro vence 3× mais que quem senta por último"* é problema de produto, não curiosidade. ⚠️ **O que está estabelecido é a ponta, não a escada:** os degraus do **meio** **não** são individualmente significativos (**#1−#2** z = 1,73; **#2−#3** z = 2,26, marginal); só o **#0−#1** é (z = 3,12). Uma das 4 rodadas ainda inverteu #1 e #2 — não cite "41/27/20/13" como escada medida. ⚠️ **NÃO se sabe se esta fatia causou ou piorou isso:** não existe medição de gradiente anterior a esta (a sonda é nova), e a fatia mudou duas coisas juntas (`encrenca` + o bot da #63). É **hipótese**, não achado causal — ver #68. **O que fecharia:** rodar o mesmo script contra o commit anterior à fatia num `git worktree`, comparando **distribuições por assento**, não taxas agregadas. 📊 **REMEDIDO em 2026-08-08** (decisão **#97**, N=480, mesa e dials de produção): **#0 30,6% · #1 27,1% · #2 22,9% · #3 19,4%** (χ² = 13,82, df=3, **p = 0,0032**). ⚠️ **E o recorte por política desarma o cherry-pick antes que alguém o faça:** dentro da política `bot` isolada (N=240) a distribuição **NÃO é significativa** (χ² = 4,60, **p = 0,20**) e o **#1 fica ACIMA do #0**. 🔴 **Nada nessa medição diz que a fatia `classe como carta` causou, aumentou ou DIMINUIU o gradiente** — não há medição pareada contra o commit pré-fatia, e comparar 2026-08-01 com 2026-08-08 move **cinco** mudanças de mesa e os quatro assentos juntos. 📊 **REMEDIDO DE NOVO em 2026-08-08** (decisão **#104**, fatia `empunhadura dupla`), em **duas sessões independentes** de N=480: **S1 #0 30,2% · #1 26,7% · #2 23,1% · #3 20,0%** (χ² = 11,22, p ≈ 0,011) e **S2 #0 26,3% · #1 30,6% · #2 23,5% · #3 19,6%** (χ² = 12,42, p ≈ 0,006). ⚠️ **A S2 põe o #1 ACIMA do #0**, e um dos seis recortes por política **não é significativo** — o único degrau que se repete em **todos** é o **#3 na lanterna**. 🔴 Nada nessa medição atribui causa a fatia nenhuma, e a comparação entre fatias **não está licenciada** ali (o controle de instrumento que licenciou o #95 era `trocaDeSlot`, que é justamente o que aquela fatia mudou). A pergunta segue **aberta e sem decisão do Pedro**. 🔴 **MUDANÇA DE ESTADO em 2026-08-10 (decisão #139): deixa de ser *"ruído sem causa e sem controle"* e passa a ser EFEITO REAL, CAUSA NÃO ISOLADA.** O que mudou não foi uma medição a mais, foi o **formato**: a fatia `consumíveis (instantâneo)` rodou **DEZ braços independentes em três sessões**, e a ordem `#0 > #1 > #2 > #3` **se repetiu nos DEZ, sem uma inversão**. Empilhado: **860 · 654 · 513 · 373 em 2.400 partidas = 35,8% · 27,3% · 21,4% · 15,5%** (χ² ≈ **216,0** com 3 g.l. — ⚠️ **derivado por aritmética das contagens publicadas**, não medido pelo harness). A dispersão do **mesmo** assento entre braços fica em ~5–9pp, contra um vão **#0−#3 de 15–25pp em cada braço**: o gradiente é maior que o ruído em toda observação. ⚠️ **Isto NÃO atribui causa, e o confundidor óbvio está NOMEADO e NÃO isolado: o assento #0 age primeiro**, e os quatro rodam a **mesma** `escolherAcao`. Nenhum braço foi desenhado para separar "vantagem de iniciativa" de qualquer outra coisa. 🔴 **E há uma distinção que NÃO pode ser borrada:** *usos de instantâneo* por assento são **RUIDOSOS** (3 das 4 observações quebram a monotonicidade); *vitórias* por assento **não são** (10 de 10). São medidas diferentes — confundi-las contamina esta pergunta. ➡️ **O que fecharia continua sendo o que já estava escrito**, agora com o alvo mais estreito: um braço que **embaralhe a ordem de ação** mantendo tudo o mais fixo. **A linha a escrever em qualquer lugar continua sendo *"o último assento vence menos"*, NUNCA a escada** | §3, §9, §12 |
| 20 | 🎚️ **O MONTANTE FICOU ESTRITAMENTE DOMINADO, e o dial não foi girado — é decisão do Pedro** (decisão **#100**, fatia `empunhadura dupla`). Com as duas mãos como vagas equivalentes, **duas Espadas Curtas dão a mesma força +4 sem o −1 de agilidade** e ocupam as mesmas duas mãos; **dois Machados do Orc num humano** (valor reduzido) também empatam em +4, e **num orc sobem para +6 com habilidade +2**. ⚠️ **A dominância é ARITMÉTICA e NÃO depende de afinidade** — o empate já acontece sem raça nenhuma. 📊 **O que está medido:** o Montante termina equipado em **34,2%–36,4% dos assentos** (n=960 por grupo, 4 grupos) — ele **não virou carta morta** sob a política de bot de hoje. 🔴 **Isso NÃO sustenta nem derruba o giro:** o bot é **guloso míope** (compara um candidato contra o ocupante de cada mão, nunca "Montante × duas espadas"), então a frequência dele em jogo **não mede** a dominância que a aritmética afirma. **O que fecharia:** girar o Montante numa fatia em que ele seja a **única** variável, remedindo força final de bot, taxa de vitória e a frequência de configuração de mãos. ⚠️ **E o giro tem uma consequência já nomeada:** o **zero** de deslocamento duplo por `trocaDeSlot` (#103) é **de política** e sai da aritmética `Montante 3 × par mais barato 4` — mexer no Montante pode virá-lo, e quem mexer **tem que remedir** aquele número. 💰 **Custo aceito por não girar agora:** uma variável por vez — girar dentro da fatia que criou a dominância seria a **terceira** dela, que é o erro das **#24/#25/#51** e o que a **#69** recusou repetir | §5, §7, §11 |
| 21 | 🔴 **REESCRITA em 2026-08-08 (madrugada): a FORMA do teto foi decidida (#107), os NÚMEROS não.** O que ficou fechado: teto vale só para **habilidade e agilidade** (as duas que rolam contra o d12 e quebram em 12; força e vida são linha reta e seguem em **modificador**), ele é **composto por raça + classe, SOMADO** sobre um teto-base, e o **máximo global é 9**. ⬜ **O que segue aberto, e é tudo decisão do Pedro:** **(a)** habilidade e agilidade ficam com **só teto** ou com **teto + modificador**? Hoje o Ladino dá `habilidade +2, agilidade +1` **direto**; com teto entrando isso vira duas manoplas no mesmo stat. 📌 **Recomendação registrada: só teto** — com base 6 e teto máximo 9 sobram **3 pontos** para o item preencher, e se a classe já entrega 2 deles a caçada de item que motiva a #107 não acontece; a sensação de *"joguei a carta e ficou forte na hora"* continua vindo de **força e vida**. ⚠️ **Risco declarado da recomendação:** um Ladino jogado sem equipamento **não faz nada visível**. **O Pedro adiou explicitamente** (*"vamos pensar nisso depois"*). **(b)** O teto máximo da **agilidade** é 9 ou **7**? 🔴 **O mesmo número não vale o mesmo nos dois stats** — habilidade 5→9 é **1,8×**, agilidade 3→9 é **3,0×**, porque a esquiva é convexa (vida efetiva `×12/(12−agi)`). Com 9 nos dois, empilhar agilidade paga quase o dobro. **(c)** Os valores por raça e por classe — 9 números novos. **(d)** Como a #107 reconcilia com o §5 (*"raça = passiva, NÃO stats"*). **O que fecharia: `grill-me`, e ele foi interrompido no meio.** 📚 Texto original de 2026-08-08 preservado abaixo, com a ressalva de que a premissa dele (esquiva pela **habilidade**) caiu com a #106 — ⚠️ **a frase *"a força NÃO paga a habilidade, quem paga é a vida"* CONTINUA VALENDO**, agora no eixo `vida × agilidade`. 🎚️ **TETO DE HABILIDADE POR RAÇA E POR CLASSE, com a habilidade se pagando em força e vida — desenho do Pedro em 2026-08-08, NÃO fechado.** A ideia, nas palavras dele: *"quanto mais hábil menos força e vida, quanto menos hábil mais força e vida"*, com raça **e** classe declarando um **teto**, e os **itens** continuando a decidir o valor dentro dele (*"eu tinha decidido que os itens vão definir e realmente vão, mas a classe e a raça definem um teto"*). Exemplos que ele deu: **Elfo Ladino** teto 9 e menos dano · **Humano Ladino** teto 8 e um pouco mais forte · **Orc Ladino** teto 6/7 e muito dano. ⚠️ **Isto muda o §5, que hoje diz que raça é *"uma passiva, NÃO stats"*** — um teto é stat, ainda que negativo, e a justificativa daquela linha (manter o Humano jogável) precisa ser reexaminada, não contornada. 📊 **A aritmética que a decisão #105 introduz e que reescreve a intuição deste desenho:** com a esquiva pela habilidade, ela faz **duas** coisas que se **multiplicam**, então um Elfo (hab. 9) contra um Orc (hab. 6) **conecta 3× mais** (37,5% × 12,5% por golpe). Para o Orc empatar, o que precisa ser **3×** maior é o **produto `vida × dano`** — algo como vida 16/dano 6 contra vida 8/dano 4 —, e **não a força sozinha**. 🔑 **A força NÃO paga a habilidade**, porque ela aumenta o quanto você tira por golpe **conectado** e não o quanto você conecta; quem paga em linha reta é a **vida**. ⚠️ **Aritmética sob premissas declaradas** (dano `level + forca`, sem passivas, sem itens equipados), **NÃO medição** — passiva e corpo mexem nisso e só o soak diz quanto. **O que fecharia:** uma sessão de `grill-me` antes de qualquer spec | §5, §11 |
| 23 | ⚔️ **O QUE ACONTECE COM O IMPACTO DO GUERREIRO (#92)?** Hoje ele é `aoEmpatarEsquiva: () => ({ empateSalva: false })` — um gancho do motor que **existe só para ele** —, e o número dele é exato: como a esquiva é `rolagem ≤ rolagem do atacante`, anular o empate troca `≤` por `<` e vale **8,3 pontos percentuais de esquiva a menos, sempre**. 🔴 **Com a #106 as duas rolagens ficam INDEPENDENTES**, e *"empate de rolagens"* vira **coincidência sem relação com a regra**, valendo `agilidade/144` — com agilidade 5, **3,5%**. Continuaria compilando, passando nos testes e **afirmando uma coisa que a regra não diz mais**. **As saídas postas na mesa, com número:** **(a)** `agilidade efetiva −1` do defensor contra ele — mesma ficção (*"o golpe é pesado demais para desviar"*), no eixo novo, e **matematicamente idêntico aos 8,3 pp de hoje**; **(b)** idem com **−2** (16,7 pp) — o Guerreiro vira a classe **anti-evasão**, papel que passa a existir de verdade; **(c)** *"o primeiro golpe dele que acerta não pode ser esquivado"*, que segue o idioma do catálogo (Casca de Pedra, Escorregadio e Explosão são todos 1×/combate); **(d)** o Impacto morre e o Guerreiro ganha outra passiva. 📌 **Recomendação registrada: (a) com `−1`**, por **método** e não por balanceamento — a fatia já troca o stat da esquiva, e o `−1` reproduz o número de hoje na casa decimal, deixando o Guerreiro como **a única peça que não muda de valor** enquanto tudo em volta muda (#24/#25/#51/#69). ⚠️ **Duas coisas que a (a) arrasta:** o gancho `aoEmpatarEsquiva` **morre com dois meses de vida**, e a agilidade efetiva precisa de **piso 0** escrito e testado — defensor com agilidade 1 contra o Guerreiro fica com **0% de esquiva**, o que é legal mas não pode ser descoberto. **O Pedro não respondeu** | §5, §7 |
| 24 | 🐲 **O CATÁLOGO DE MONSTROS FICA FÁCIL DEMAIS COM A #106 — e ninguém decidiu o que fazer.** A regra velha amarrava a esquiva do defensor à **habilidade de quem ataca**, e os monstros (habilidade **2–4**) ganhavam com isso uma proteção **de graça**: um atacante desajeitado só acerta rolando baixo, e rolagem baixa era difícil de esquivar. Qualquer regra que ponha o stat no defensor **tira essa proteção**. 📊 **Aritmética, não medição** — jogador nu × Goblin, ataques até matar: **hoje 14,1 × 13,7** (moeda ao ar) → **#106 15 × 20,6** (o jogador ganha folgado). ✅ **Em compensação, a #106 dá variedade DE GRAÇA:** hoje os cinco monstros esquivam todos na **mesma taxa de 29,2%** (a `agilidade` da carta deles não faz nada), e com a #106 elas passam a valer — **Ogro 17% · Rato 25% · Goblin e Carniçal 33% · Lobo Sombrio 58%** —, então cada monstro passa a pedir uma coisa diferente **sem uma carta nova**. **O que fecharia:** rever a statline dos 5 monstros na mesma fatia da #106, com soak, e decidir se o alvo é *"devolver o patamar de hoje"* ou *"o jogo era duro demais e está bom assim"* | §4, §5, §13 |
| 25 | 👁️ **O JOGADOR NÃO CONSEGUE PERCEBER O EFEITO DO CONSUMÍVEL QUE ACABOU DE QUEIMAR — achado do GATE OCULAR de 2026-08-10, não de teste nem de revisão.** O Pedro usou a **Areia nos Olhos** e o **Óleo de Precisão** numa partida real e relatou *"não consegui verificar perfeitamente seu funcionamento… não consegui reparar"*. ✅ **O domínio está CERTO** — dirigindo o servidor vivo pelo HTTP real: Areia ⇒ força do monstro **5 → 3** com o lado do jogador intacto; Óleo ⇒ habilidade **6 → 8**; Poção com vida cheia ⇒ **400** do guard de desperdício. **O que falha é a COMUNICAÇÃO.** Duas causas de famílias diferentes, e elas não se substituem: **(a) APRESENTAÇÃO** — o painel de combate é **uma linha corrida com ONZE números** (`Você: 10 / 10 · força 3 · habilidade 6 · agilidade 5 · Carniçal: 16 de vida · força 5 · habilidade 4 · agilidade 4 · …`), e um instantâneo muda **um dígito no meio dela**, sem nada marcar o que mudou; a Areia ainda muda o número do **monstro**, o sexto da frase. **(b) 🔑 O ÓLEO DE PRECISÃO É IMPERCEPTÍVEL POR CONSTRUÇÃO, e isso é DESIGN, não UI:** `habilidade +2` move a chance de acertar de **6/12 para 8/12** e **não tem nenhuma consequência observável num clique** — só ao longo de muitas rolagens. Das quatro cartas ele é a única assim; força vira dano na hora, vida vira vida na hora, e a Areia teria a mesma legibilidade da força se fosse apresentada. ⚠️ **A calibragem congelada (#130) escolheu os quatro números para serem DISTINTOS entre si e nenhum critério foi de perceptibilidade** — a pergunta *"o jogador sente esta carta?"* nunca foi feita. 📊 **Contexto medido:** o consumível é queimado **3,40 vezes por jogador por partida**; não é evento raro cuja opacidade se perdoe. **As saídas candidatas, de camadas diferentes:** **(a)** realçar o stat alterado no painel, ou mostrar o delta (`habilidade 8 (+2)`) por um instante — barato, resolve a apresentação e **não** resolve o Óleo; **(b)** o log dizer o efeito e não só a carta (*"Você usa Óleo de Precisão: habilidade 6 → 8"*) — o log já é o lugar onde o jogador procura o que aconteceu; **(c)** trocar o **efeito** do Óleo por um que se sinta num clique, aceitando que a habilidade não é matéria de consumível; **(d)** aceitar como custo. ⚠️ **Nenhuma foi decidida, e a (c) mexe na calibragem congelada.** 🔑 **O que este item prova sobre o PROCESSO: é a terceira vez que o gate ocular acha *"o código faz certo e não conta a ninguém"*** — e desta vez o defeito não é um render faltando (que a revisão do branch pegou horas antes, no log narrando pelo id), é a **legibilidade do que já é renderizado**, que nenhum instrumento automático alcança | §5, §7, §11 |
| 22 | ✅ **RESPONDIDA em 2026-08-08 (madrugada) pela decisão #106: quem esquiva é a AGILIDADE.** ⚠️ Fica registrada em vez de removida porque o histórico importa — a **#105** havia escolhido **habilidade** e durou menos de um dia. 🔑 **O argumento que decidiu:** o problema nunca foi o dado, era **um stat com dois trabalhos**; com a agilidade esquivando, cada stat fica com um (habilidade acerta · agilidade esquiva · força dana · vida dura) e o composto de 3× **some**. ⬜ **A segunda metade da pergunta — o Impacto do Guerreiro — NÃO foi respondida e virou a pergunta 23.** Texto original abaixo, preservado: a decisão **#105** escolheu **habilidade**, e o Pedro reabriu a pergunta na mesma sessão ao ver o efeito composto (*"vamos pensar em uma nova mecânica de batalha para deixar justo"*). 🔑 **A escolha é estrutural, não de dial:** com a habilidade fazendo acerto **e** esquiva, os dois efeitos **se multiplicam** e a habilidade vira o stat dominante — é isso que força o produto `vida × dano` a ser 3× (pergunta 21). Com a **agilidade** esquivando, os quatro stats ficam com um trabalho cada (habilidade = acertar · agilidade = esquivar · força = dano · vida = durar), o composto **some**, e o desenho da pergunta 21 vira uma escolha de dial em vez de uma restrição aritmética. ⚠️ **E o spec original PREVIU exatamente isso:** a Decisão 9 escreve *"candidato a refinamento futuro: dar papel à Agilidade"*. Hoje a agilidade só decide **iniciativa** — é o stat mais magro do jogo. 💰 **O custo da agilidade:** ela deixa de ser "só iniciativa" e passa a valer muito, o que mexe em todo item que dá agilidade (a Cota de Malha e a Placa do Clã dão **−1**) e no `montarCombatente` (o `PISO = 1`). ⬜ **E o Impacto (#92) fica pendurado nas duas saídas:** com a habilidade, ele morde na proporção da habilidade do defensor; com a agilidade, o empate perde qualquer relação com a condição de esquiva e vira coincidência pura. **O Pedro adiou:** *"pensamos melhor se a passiva do guerreiro ainda vale a pena"*. **O que fecharia:** `grill-me`, junto com a pergunta 21 — as duas são a mesma conversa | §5, §7 |
| 18 | 🎚️ **A `MARGEM_DE_ENCRENCA` (1,2) está FROUXA — está MEDIDO, e o Pedro decidiu NÃO girá-la agora (decisão #69).** Três números independentes sustentam a leitura, todos do fechamento do gate do Plano 4b (5 rodadas de 80 partidas, dials de produção, dado e embaralho reais): **(a) ela quase não morde** — reprova **5,5%–7,4%** dos candidatos individuais e produz recusa em **~1,1% das entradas** com monstro na mão (**7/707** e **8/672**, R4/R5); **(b) o ganho ex-post é modesto** — o bot perde **8,69%** das lutas que ele mesmo ESCOLHEU (**236/2717**) contra **11,46%** das FORÇADAS pelo `vasculhar` (**919/8019**, grupo de controle, empoçado **R2–R5, N=320 partidas**): a diferença é real (**z = 4,03**) mas vale só **1,32×**, e ele ainda perde **1 em cada 12** lutas que julgou favoráveis; **(c) a curva do dial põe 1,2 no fundo plano da escala** — % das entradas com monstro na mão que virariam recusa, **R4/R5 (N=160)**: **1,2 → 0,99% / 1,19%** · **1,5 → 4,81% / 4,46%** · **2,0 → 15,42% / 14,58%** · **3,0 → 36,92% / 35,42%**. Ir a **1,5** multiplicaria a recusa por ~4 sem tornar o bot medroso. ⚠️ **O que NÃO está quebrado, e precisa ser lido junto:** **ex-ante deu zero em 3.421 aceites** com desvantagem (N=400) — o bot **nunca** aceita luta que a própria fórmula diz que perde, e a menor razão observada num aceite foi **1,33**. O que a margem paga é a fórmula ser otimista **de propósito** (ignora esquiva e passivas de raça — #63), e **1,2 paga pouco**. 📌 **Nota de método:** as **recusas** saem da `escolherAcao` real; os itens (a) e (c) usam uma **cópia** da fórmula (ela é privada em `bot.ts`), **verificada** contra a política real (a curva previu 7 e 8 recusas em duas rodadas e a política produziu 7 e 8); o item (b) **não usa fórmula nenhuma**. **O que fecharia:** girar a margem numa fatia em que ela seja a **única** variável, remedindo força final dos bots, taxa de vitória e derrota ex-post nas lutas escolhidas — hoje isso é impossível de ler, porque a `encrenca` e o bot da #63 entraram **juntos** (ressalva-mãe da sessão de 2026-08-01). 🔴 **ATUALIZAÇÃO 2026-08-08 — a margem ficou MAIS FROUXA, e por dedução do código, não por medição nova.** A #63 declara que `rodadasParaMatar` é **otimista de propósito**: ela ignora esquiva **e passivas**, e a `MARGEM_DE_ENCRENCA` existe para pagar esse otimismo. A fatia `classe como carta` acrescentou **uma segunda passiva por combatente** (#92) — o lutador agora pode ter raça **e** classe —, então há **mais** efeito de combate fora da fórmula e a mesma margem de 1,2 paga **menos** do que já pagava pouco. ⚠️ **Isto NÃO foi medido:** o soak de 2026-08-08 não instrumentou recusas nem derrota ex-post, e `rodadasParaMatar`/`MARGEM_DE_ENCRENCA` continuam **privados** de `bot.ts`. É consequência lida no código, e o que fecharia o dial continua sendo o que já estava escrito acima — com **uma variável a mais** para isolar agora. 🔴 **ATUALIZAÇÃO 2026-08-09 — a fatia `Bad Stuff e evacuação` AGRAVA o dial, e isto estava declarado no §10 do spec dela ANTES de ser construída.** `rodadasParaMatar` estima **quanto custa vencer**, e **nada** na fórmula sabe **quanto custa PERDER** — que, a partir desta fatia, deixou de ser zero: contra os quatro monstros comuns é um encaixe arrancado direto ao cemitério, e **contra o Ogro é perder tudo** (mão, mochila e os cinco encaixes). ➡️ O bot aceita luta contra o Ogro com **exatamente a mesma margem** de antes, e o Ogro é o único monstro cujo Bad Stuff é catastrófico. ⚠️ **Deduzido do código, NÃO medido:** o soak da 2a **não instrumentou recusas nem derrota ex-post** (as funções seguem privadas de `bot.ts`), e a política do bot **não foi tocada** — o que muda é a **consequência** de uma decisão que ele continua tomando do mesmo jeito. 📌 **Não é bug daquela fatia; é dívida que ela agrava**, e agora o dial tem **duas** dimensões a isolar (o otimismo da fórmula sobre o combate **e** a cegueira dela sobre o preço da derrota). 🔴 **ATUALIZAÇÃO 2026-08-10 — a fatia `consumíveis (instantâneo)` agrava numa TERCEIRA direção, e ela puxa para o lado OPOSTO das outras duas.** `rodadasParaMatar` estima o combate pelo **corpo equipado** e **não sabe que o lutador tem uma poção na mochila** — então agora o bot **subestima** a própria chance, depois de duas atualizações em que ele a superestimava. ⚠️ **As três se cancelam em direção desconhecida e nenhuma foi medida:** o soak da 2b **não instrumentou recusas nem derrota ex-post** (as funções seguem privadas de `bot.ts`). 📊 **O que existe de contexto: 3,40 usos de instantâneo por jogador por partida**, ou seja o recurso ignorado pela fórmula é **frequente, não marginal**. **Deduzido do código, NÃO medido** | §6, §13 |
| 19 | 🔴 **CARTA PROIBIDA GUARDADA NA MOCHILA FICA PRESA ATÉ O FIM DA PARTIDA, e ninguém decidiu se isso é aceitável.** Medido em 2026-08-02 (fatia `afinidade`): **8,0–8,1 cartas presas por mesa de 4** no fim da partida (dispersão real por rodada **7,8–8,2**; **N=480** no baralho de produção; **zero em 480** no controle de 8 ids, que é zero **estrutural** — não há exclusivo lá). São ~2 por jogador contra um `LIMITE_MOCHILA` de **5**: ≈**40% da capacidade de mochila da mesa** ocupada por carta que nunca vai sair. ⚠️ **NÃO é bug, e a distinção é o ponto:** `guardarCarta` **corretamente não tem guard de afinidade** — guardar um exclusivo de outra raça é jogada legítima para quem pretende trocar de raça depois. O que fecha a armadilha é a soma de **duas** coisas: (1) o fallback de guardar do bot (`bot.ts:251`, `vista.suaMao.find((c) => c.tipo === 'equipamento')`) **não olha afinidade**, e (2) **mochila → mão não existe** nesta fatia. Para um bot, uma vez presa, presa para sempre. 🔴 **NÃO foi corrigido de propósito:** girar a política do bot no fechamento desta fatia seria a **TERCEIRA** variável dela, que é literalmente o erro catalogado pelas #24/#25 e repetido pela #51 — e recusado pela #69 uma fatia atrás. **A leitura é do Pedro.** As saídas candidatas são de famílias diferentes e não se substituem: **(a)** ensinar o bot a não guardar proibido (política, barato, não muda regra); **(b)** o verbo **mochila → mão**, que já está adiado para a fatia da interferência (regra nova); **(c)** aceitar como custo de desenho — a mochila entulhada É a punição por guardar o que não se pode vestir. ⚠️ **NÃO tocada na fatia `classe como carta` (2026-08-08), e nem remedida** — mas **uma premissa do texto acima envelheceu**: o `LIMITE_MOCHILA` constante **não existe mais** (virou `limiteDeMochila(jogador)`, #89), então o denominador *"~2 presas por jogador contra um teto de 5"* passa a ser **5 ou 6** conforme a classe esteja em jogo. Como **93,5% dos assentos terminam com classe** (#96), o **5** continua sendo o teto do fim de partida na esmagadora maioria dos casos, e a leitura *"≈40% da capacidade"* **não muda materialmente** — mas quem for remedir tem que ler o teto do estado, nunca de uma constante. 🔴 **ATUALIZAÇÃO 2026-08-10 — nasceu uma SEGUNDA fonte de carta parada na mochila, e ela é de outra família.** A fatia `consumíveis (instantâneo)` fez a mochila aceitar consumível (#131), e a mochila é **isenta do limite de mão**: estocar poção sai **de graça**. 📊 **Medido, e o risco é MODERADO na dose de produção: 1,98 instantâneo por partida (0,50 por jogador, 12,4% da tiragem de 16) termina dormindo na mochila** — contra **65,0%** que terminam nos baralhos, isto é, circularam (N = 16 cartas × 240 partidas, braço B). 🔴 **E ele quase DOBRA quando a dose sobe: 3,49 por partida no braço D** (33,3% de consumível num baralho de 48) — a mesma escada de aderência da #137. ⚠️ **A diferença que impede colapsar as duas fontes numa só:** o exclusivo proibido fica preso **para sempre** (falta o verbo mochila → mão), e o consumível fica preso **até o combate seguinte** — ele tem verbo (`usarInstantaneo`, da própria mochila) e **75,6% dos usos saem de lá**. São o mesmo sintoma com prognósticos opostos, e **não há guard contra nenhuma das duas** — há instrumento | §5, §11, §13 |
...
| 46 | **Maldições / Bad Stuff ganham FATIA PRÓPRIA, logo depois do 4b — e antes da interferência e do frontend animado** | Duas razões que se somam. **(1) É a primeira carta que mira OUTRO jogador.** Hoje tudo é auto-dirigido (equipar, guardar, jogar raça); só a caridade toca outro, e ali quem escolhe é a regra, não o jogador. Mirar exige encanamento novo — seletor de alvo, validação, evento de duas pontas (o `participantesDe.ts` já sabe: a `entrega` aparece nos dois filtros). Se a interferência viesse antes, ela introduziria **cinco eixos de uma vez** (timer, concorrência, mira, motor para N, três famílias de carta); a maldição resolve **um** deles sozinho, sem timer, sem concorrência de combate, sem rede. **(2) 🔴 É o conserto da economia.** O §10 promete *"morte = evacuação: perde TODAS as cartas"* — o **maior caminho de volta do jogo** — e ele **não existe em código**: perder combate só incrementa um contador (`mesa.ts:1312` — âncora: o `derrotas + 1` dentro de `fecharCombate`; ✏️ citado como `:939` até 2026-08-09, quando a revisão do spec da 2a reconferiu). Sem essa fatia, o baralho de Itens continua secando em **20/20 partidas**. Ela não é conteúdo: é a peça que fecha o loop. **Antes do frontend animado** porque a fatia animada deve desenhar o jogo inteiro — maldição chegando depois chega sem animação, e alguém volta para fazê-la |
...
✏️ **ATUALIZAÇÃO MARCADA (2026-08-08): o gate FOI rodado e APROVADO** — o Pedro percorreu os 7 itens
e reportou *"está certinho"*. 🔑 **E achou um bug que 661 testes e três revisões amplas não acharam**
(*"consigo usar um machado de orc e um escudo, mas não consigo usar dois machados"*), **pré-existente
desde o Plano 3a** e previsto pela **#39** — virou a fatia `empunhadura dupla` (#98–#104). ⚠️ **O
parágrafo acima fica intacto de propósito:** ele registra o que era verdade quando foi escrito, e é
ESTA linha que diz o presente.
...
### Sessão de 2026-08-08 — a `empunhadura dupla`: as duas mãos viram a mesma vaga

**A fatia está construída** (branch `feat/empunhadura-dupla`, 6 tasks — 4 de código, uma de soak e
esta de documentação —, **693 testes verdes** (motor 56 · cartas 52 · personagem 11 · partida 352 ·
shared 23 · server 29 · web 170), typecheck 7/7, lint limpo). Ela nasceu do **gate ocular do Pedro**
na fatia anterior: *"consigo usar um machado de orc e um escudo, mas não consigo usar dois
machados"*. Spec `docs/superpowers/specs/2026-08-08-empunhadura-dupla-design.md`.

🔴 **O gate ocular do Pedro NÃO foi rodado quando estas linhas foram escritas.** O roteiro (**5**
itens, cada um com a **frequência esperada** declarada) está no `CLAUDE.md`, e **nenhum item foi
conferido**. Esta seção descreve o que foi **construído e medido**, não o que um humano observou na
tela.

| # | Decisão | Porquê |
|---|---|---|
| 98 | **AS DUAS MÃOS SÃO VAGAS EQUIVALENTES.** Todo item de uma mão — arma **e** escudo — declara `'mao'`, e o corpo continua com os cinco encaixes físicos: `SlotDeItem` (`capacete｜armadura｜mao｜pes`) é o que o **ITEM** declara, `Slot` é o que o **CORPO** tem. **Consequência aceita: dois escudos passa a ser jogada legal** | O bug do Pedro **não era de código — era o modelo de dados**: `ItemCarta.slot` é um valor **único**, e as **três** armas do catálogo declaravam `maoDireita`, enquanto `maoEsquerda` tinha **exatamente uma** opção no jogo inteiro. O jogador não via *"o machado é especial"*, via *"só existe um slot de arma, com dois nomes de mão por cima"*. ⚠️ **Não é regressão de fatia nenhuma:** o comportamento existe desde o Plano 3a, quando o baralho de Tesouros nasceu, e o código estava **fiel ao §5 escrito** — é o **escrito** que esta fatia muda. 🔑 **As duas alternativas foram recusadas com motivo:** *(B) uma união só com capacidade* (`maos: [Carta｜null, Carta｜null]`) é o modelo **mais honesto** e quebraria `Record<Slot, …>`, `itensEquipados`, `tirarDosSlots`, a projeção, a tela e a suíte inteira de equipar — fatia muito maior por pureza; *(C) `slot` fixo + `flexivel: boolean`* é **a declaração mentirosa que causou o bug, agora com uma flag por cima**. 💰 **Custo aceito e declarado:** de **duas** uniões duplicadas (`Slot`, travada por `_CoberturaSlot`) para **quatro**, com um guard novo **obrigatório** (`_CoberturaSlotDeItem`) — sem ele as duas cópias de `SlotDeItem` divergiriam em silêncio, que é o defeito exato que o guard antigo existe para impedir |
| 99 | **O JOGADOR ESCOLHE A MÃO ALVO NA PRÓPRIA AÇÃO, não numa pendência.** `equiparCarta` ganha `mao?: 'maoDireita'｜'maoEsquerda'`. **As quatro regras, em ordem:** item que **não** é de mão → campo **ignorado**; item de mão com `duasMaos` (o Montante) → campo **ignorado**, ocupa as duas; **ao menos uma vaga livre** → campo **opcional** (omitido, preenche a livre; presente, respeita a escolha); **as duas ocupadas** → campo **OBRIGATÓRIO**, e omitido é `AcaoInvalida` (400) | Preserva a agência da **#59** (*"o jogo não escolhe por você"*) **sem** criar a **4ª pendência** do jogo. Uma pendência custaria estado novo, verbo novo, o bot respondendo **antes** do `switch` e a tela apagando tudo — e ainda **encadearia** com a queima (escolher a mão, depois escolher o que queimar). A escolha acontece no clique. ⚠️ **A regra 3 tem uma armadilha, escrita no spec ANTES de alguém cair nela:** `mao` apontando para uma mão **ocupada** enquanto a outra está **livre** é escolha legítima (o jogador quer trocar *aquele* item), **não** erro — nada de guard exigindo vaga livre. 🔑 **A regra 4 é o par fino novo, e são DUAS linhas da tabela do `aplicarAcao`, não uma**, porque `equiparCarta` é legal nas **duas** fases paradas — mesma convenção da `afinidade`. ✅ **Recontagem a partir do REDUCER, `AcaoInvalida` por `AcaoInvalida`: de 16 pares em 19 linhas para 18 pares em 21 linhas**, conferida de forma independente pelo revisor (17 `AcaoInvalida` mais a conversão de `AcaoIlegal` do motor). ⚠️ **A recontagem sai do código para a tabela, nunca ao contrário** — essa tabela já mentiu **quatro** vezes, três por agrupar duas fases numa célula e uma por **omissão**, mais um erro de **inflação** |
| 100 | 🎚️ **O MONTANTE FICA ESTRITAMENTE DOMINADO, E ISSO É ACEITO POR UMA FATIA.** Duas Espadas Curtas dão a **mesma força +4** sem o **−1 de agilidade** e ocupam as mesmas duas mãos; dois Machados do Orc num **humano** (valor reduzido) também empatam em +4, e num orc sobem para **+6 com habilidade +2**. O dial fica para uma passada **separada**, com número medido na mão | **Uma variável por vez.** Girar o Montante dentro da fatia que criou a dominância seria a **terceira** variável dela (a mecânica e a política do bot já entram juntas), e é literalmente o erro que as **#24/#25** catalogaram, a **#51** repetiu e a **#69** recusou repetir há duas fatias. 🔑 **A dominância NÃO depende de afinidade** — o empate acontece já num humano sem raça nenhuma. 📊 **E o soak dá o número que impede a leitura preguiçosa dos dois lados:** o Montante termina equipado em **34,2%–36,4% dos assentos** (n=960 por grupo), ou seja **não virou carta morta** sob a política de bot de hoje. ⚠️ **Isso não sustenta nem derruba a passada de dial sozinho:** a dominância é **aritmética**, e o bot é **guloso míope**, não um otimizador de configuração — ele não compara *"Montante"* contra *"duas espadas"*, compara um candidato contra o ocupante de cada mão. **A leitura do dial é do Pedro** — pergunta **20** do §18 |
| 101 | ✅ **A QUEIXA DA #39 SOBRE AS MÃOS MORRE SEM GIRAR DIAL NENHUM — e CONTINUA DE PÉ para `pes`.** A #39 reclamava que *"são 5 slots; com 8 itens vários slots têm uma opção só, e aí não existe a decisão que o §5 diz ser a razão dos slots"*. A família "mão" passa de **3 + 1** (três armas presas à direita, o escudo preso à esquerda) para **4 itens numa vaga dupla**. **`pes` continua com 2** | A #39 é dial de **variedade de catálogo** (equipamento **15**, era 8), e a `afinidade` levou o catálogo a 12 itens com `maoEsquerda` **ainda em 1** — a previsão estava escrita desde 2026-07-29 e **ninguém remediu**. 🔑 **O que esta decisão registra é que a queixa tinha DUAS causas e só uma era o número de itens:** a outra era a **repartição**, e essa se conserta sem carta nova. ⚠️ **Escreva as duas metades:** dizer *"a #39 fechou"* faria o leitor futuro parar de contar `pes`, que segue com uma escolha binária; e dizer *"nada mudou, o catálogo tem os mesmos 12 itens"* apagaria o efeito real — **as mesmas cartas, uma repartição diferente** |
| 102 | 📊 **A MECÂNICA NOVA É USADA, E A RESPOSTA HONESTA É UM INTERVALO, NÃO UM NÚMERO.** No estado final, por assento (n=960 por grupo, 4 grupos): **LIMITE INFERIOR 12,4%–15,0%** dos assentos terminam com **duas cartas distintas nas mãos e nenhuma delas o escudo** — configuração **impossível antes desta fatia**; **LIMITE SUPERIOR 51,7%–54,5%** terminam com duas cartas distintas, mas isso **inclui `arma + escudo`, que já era possível**. As duas mãos terminam ocupadas em **87,3%–89,1%** dos assentos. Por AÇÃO (N=240 por grupo): `equiparCarta` com o campo `mao` preenchido em **4,52%–4,82%**; `equipou` em `maoEsquerda` com item que **não** é o escudo em **7,43%–8,11%**. **Regressão:** `AcaoInvalida` (bot **e** humano), `Error` cru, teto de 30.000 ações e **beco sem saída** deram **zero em 960 partidas**; o **censo de conservação id-a-id depois de CADA ação** deu **zero falhas em 352.460 censos**, com **N=960** | ⚠️ **Os dois limites têm o MESMO denominador e numeradores diferentes: são medidas DIFERENTES, e colapsá-las num número só é a armadilha dos "~72% e ~96%" do Plano 4b.** ⚠️ **`mao` preenchido (≈4,5–4,8%) e `maoEsquerda` (≈13%) também não se colapsam:** a primeira é *"quantas vezes o jogador teve que ESCOLHER"* (as duas ocupadas); a segunda é *"quantas vezes o item acabou na esquerda"*, que na maioria é a mão livre se resolvendo sozinha. 🔴 **O limite inferior é conservador por DOIS motivos declarados:** o **escudo caindo na mão DIREITA** e **escudo + escudo** também são configurações novas desta fatia e **não foram contadas**. 🔑 **O zero do censo só vale por causa do GATE que rodou ANTES dele:** quatro smoke tests provaram que o censo **enxerga cartas DISTINTAS nas duas mãos** (sabotá-lo acusa o id sumido) e que ele **deduplica a arma de duas mãos por id** (sem a dedup, acusa a duplicata) — esta fatia põe cartas diferentes nas duas mãos como estado de **rotina**, e um censo indexado por **nome de slot** em vez de por **id de carta** esconderia uma perda real bem aqui. Foi `emJogo.raca` que o script do Plano 4a esqueceu: **um zero de conservação sem esse gate não vale nada** |
| 103 | 📊 **A PREVISÃO DO §8.2 DO SPEC NÃO SE CONFIRMOU: o deslocamento duplo por `trocaDeSlot` continua em ZERO — e o zero é DE POLÍTICA, não de impossibilidade.** O spec previu que esta fatia tornaria alcançável o cenário que a **#86** declarou candidato a inexercitável pelo fixture. Medido: **zero filas ≥2 em 1.194 aberturas por `trocaDeSlot`**, e mais fundo, **zero ações deslocaram ≥2 itens por `trocaDeSlot` em 3.859 ações que deslocaram ≥1** (N=960). No mesmo instrumento, `perdeuAfinidade` deslocou ≥2 em **162 de ≈620** ações (limites exatos **608–627**, ≈26%) e produziu **46 de 201** aberturas com fila ≥2; `mochilaEncolheu` deu **zero em 393**, e esse é **ESTRUTURAL** | 🔑 **Medir só a FILA deixaria o zero ambíguo** (*"o deslocamento duplo não acontece"* ou *"acontece e a mochila tinha vaga"*?): o contador profundo separa os dois. ⚠️ **As duas linhas são medidas com denominadores DIFERENTES** — *"aberturas"* (1.194) conta só as vezes em que a mochila estava cheia; *"ações que deslocaram ≥1"* (3.859) conta todo deslocamento. 🔴 **DUAS evidências INDEPENDENTES sustentam "de política", e é a combinação que decide:** (1) o **caminho de código existe e foi percorrido** — o smoke monta o estado à mão e obtém `fila = 2`, o que prova alcançabilidade **pelo fixture**, não pelo jogo; (2) a **pré-condição é COMUM em jogo real** — 51,7%–54,5% dos assentos terminam com duas cartas distintas nas mãos (#102). O que não acontece é o bot escolher equipar o Montante por cima disso. ⚠️ **O MECANISMO é DEDUZIDO do catálogo e do `>` estrito de `vestirOuGuardar`, NÃO medido:** o Montante vale `4 + (−1) = 3` e o par de mãos ocupadas mais barato custa `2 + 2 = 4`. ➡️ **Se o dial da #100 girar, este zero pode virar, e quem girar tem que remedir.** 🔴 **E isto NÃO resolve a ambiguidade da #86:** aquele zero é sobre **os dados dela**, num build com catálogo, política de bot e caminho de deslocamento diferentes — resolvê-la exigiria rodar o contador profundo **contra o build dela**, e a comparação entre fatias **não está licenciada** (ver a ressalva abaixo). ⚠️ **O zero de `mochilaEncolheu` é ESTRUTURAL e não herda nada disto:** o teto encolhe **6 → 5**, exatamente **uma** vaga, então o excedente é **sempre 1** por construção — **não escreva "raríssimo"**, o leitor futuro pularia o teste do único caminho em que a fila mista importa |
| 104 | 📊 **O gradiente de assento, REMEDIDO e SEM CAUSA ATRIBUÍDA:** duas sessões independentes, as duas políticas somadas — **S1: #0 30,2% · #1 26,7% · #2 23,1% · #3 20,0%** (χ² = 11,22, df=3, p ≈ 0,011) e **S2: #0 26,3% · #1 30,6% · #2 23,5% · #3 19,6%** (χ² = 12,42, p ≈ 0,006), **N=480 cada** | Registro para a **pergunta 17 do §18**, que é **medição, não decisão** desta fatia. 🔴 **Nada aqui diz que esta fatia causou, aumentou ou DIMINUIU o gradiente** — não há medição pareada contra o commit pré-fatia, e essa conclusão já foi escrita e **derrubada em revisão** numa fatia anterior, por cherry-pick de baseline. ⚠️ **Escreva *"o último assento vence menos"*, NÃO a escada:** em **dois** dos seis recortes o **#1 fica ACIMA do #0** (S2 agregado e S2 `nunca-guarda`), e um recorte inteiro (S1 `nunca-guarda`, χ² = 3,30) **não é significativo**. O único degrau que se repete em **todos** os recortes é o **#3 na lanterna**. ⚠️ Os `p` são **aproximados** (χ² de aderência a 25%, sem correção para múltiplas comparações), e os recortes por política **não são independentes** do agregado da mesma sessão |

### Sessão de 2026-08-08 (fim do dia) — a esquiva deixa de ser pura

| # | Decisão | Por quê |
|---|---|---|
| 105 | 🎲 **A ESQUIVA DEIXA DE SER PURA: quem decide é o stat do DEFENSOR, não a rolagem do atacante.** O defensor esquiva se `rolagem ≤ habilidade DELE`. 🔴 **Isto REVOGA a "Decisão 9 — Esquiva pura" do spec original** (`docs/superpowers/specs/2026-07-17-card-dungeon-design.md:48`), que dizia *"o stat do defensor **não** influencia na v1"* e que está implementada hoje em `packages/motor/src/ataque.ts:29` (`esquivou = rolagem <= rolagemAtaque`). ⚠️ **O ataque NÃO muda:** o atacante continua acertando com `rolagem ≤ habilidade dele`. A rolagem do atacante **continua viajando** no combate, mas só para o **Impacto** detectar o empate. ⬜ **NÃO CONSTRUÍDA** — vai junto com a fatia **Maldições / Bad Stuff** | Pedido do Pedro em 2026-08-08: *"a esquiva não é sobre tirar o menor número do dado… é um valor dentro da habilidade, se ele não foi hábil ele erra a esquiva"*. **O que a regra antiga fazia de estranho:** a esquiva do defensor era função da **rolagem de quem o atacava** — um golpe de um atacante desajeitado (rolagem baixa) era **mais fácil** de esquivar que o de um habilidoso, e o defensor não tinha stat nenhum no assunto. 📊 **CONSEQUÊNCIA ARITMÉTICA, não medida:** a chance de esquivar sai de `E[rolagem do atacante]/12` para `habilidade do defensor/12` — para habilidade 6 dos dois lados, de **~29% para 50%**. O combate fica **bem mais longo**. 🔴 **E a habilidade passa a fazer DUAS coisas que se MULTIPLICAM** — acertar e esquivar: um lutador de habilidade 9 contra um de 6 **conecta 3× mais** (`9/12 × 6/12 = 37,5%` contra `6/12 × 3/12 = 12,5%`). ⚠️ **Duas coisas que esta decisão QUEBRA e que precisam ser lidas junto:** (1) a **#63** justifica ignorar a esquiva no `rodadasParaMatar` dizendo que *"habilidade baixa acerta pouco, mas acerta com rolagem baixa, difícil de esquivar"* — **essa assimetria DEIXA DE EXISTIR**, e a `MARGEM_DE_ENCRENCA` (pergunta 18) fica ainda mais frouxa; (2) o **Impacto** do Guerreiro (#92) muda de sentido sem mudar de código — hoje o empate é a **fronteira** da condição de esquiva, e passa a ser **coincidência** entre dois dados independentes, mordendo só quando o empate cai dentro da habilidade do defensor. 💰 **Custo de método ACEITO e declarado:** vai junto com as Maldições, que é a fatia que encara a economia (pergunta 11) — logo **nenhum número vai isolar economia de duração de combate**. É a **#51** conscientemente repetida, com o Pedro avisado antes de decidir |

🔴 **RESSALVA-MÃE, válida para TODOS os números das #100–#104:** esta fatia mudou **duas coisas ao
mesmo tempo** — a **mecânica** (um item de uma mão cabe em qualquer das duas, e o jogador escolhe
qual liberar) **e a política do bot** (`vestirOuGuardar` passou a avaliar **as duas** mãos e a
escolher a de maior ganho estritamente positivo). A mudança de política é **forçada pela mecânica**,
não um dial independente — mas **nenhum número isola uma da outra**. E os 3 bots rodam a **mesma**
`escolherAcao` da política `bot` do humano, então **toda comparação contra fatia anterior move os
QUATRO assentos juntos**. É a **#51**, que era a **#24/#25**, que a **#69** recusou repetir.

🔴 **E há uma segunda ressalva que esta fatia NÃO consegue pagar: COMPARAÇÃO ENTRE FATIAS NÃO ESTÁ
LICENCIADA.** A fatia anterior licenciou a dela publicando um **controle de instrumento** —
...
substituto sobreviveu ao exame: `mochilaEncolheu` e `perdeuAfinidade` dependem da ocupação da mochila
e de quais exclusivos estão vestidos — as duas, consequência direta da política de equipar —, e toda
medida de combate é a jusante do corpo equipado. ➡️ **Os baselines (força final de bot 5,98–6,34 e
6,82–7,00; aberturas de queima 1,86/partida e 0,465/jogador; ritmo 95 · 89,5 · 94) aparecem como
CONTEXTO, nunca como comparação.** ⚠️ E vale sempre: **um controle licencia a comparação, nunca a
atribuição de causa.**
✅ **O que ESTÁ licenciado é o controle INTERNO S1 × S2** — mesmo build, mesma sessão, mesmo
instrumento, só o acaso mudando: a **maior diferença OBSERVADA** entre as duas sessões, em cinco
sub-medidas, foi **6,9%**. ⚠️ **Isso NÃO é um limiar estimado de ruído** (são 5 observações de UM par
de sessões, sem intervalo de confiança, e a maior de 5 subestima a dispersão real por construção) —
use como *"a esta N, diferenças desta ordem já foram produzidas só pelo acaso"*.

⚠️ **"zero em N partidas", NUNCA "não acontece"** — é checagem depois de cada ação nas condições
medidas (mesa de 4, humano no assento #0, patente-alvo **importada** de `PATENTE_ALVO_PADRAO`, mão
inicial 4+4, **68 Portas + 48 Tesouros**, dado e embaralho reais **sem semente**), não prova de
impossibilidade. ⚠️ **Cada linha carrega o SEU N; não empreste o N de uma linha para outra.**
🔴 **O relatório do soak e o `soak.ts` moram em `.superpowers/sdd/`, que é GITIGNORED — estes números
só existem aqui e no `CLAUDE.md`.** Os harness do Plano 4b, da `afinidade`, da `escolha do descarte`
e da `classe como carta` **já sumiram**; este foi escrito do zero pela **quarta** fatia seguida.

### 🔬 O que a execução pegou, e que vale além das decisões

- 🔴 **O RISCO Nº 1 DA FATIA — o LOOP DE TROCA — foi descartado com a MÉTRICA CERTA, depois de quase
  ser descartado com a errada.** A `afinidade` mediu que afrouxar o `>` estrito de `vestirOuGuardar`
  para `>=` **trava a partida** (ritmo 179–207 contra ~105), e com **duas** mãos candidatas o risco
  **aumenta**. ⚠️ **A mediana de ações do HUMANO é a métrica MENOS sensível a esse risco**: um loop
  num assento de **bot** infla o total **sem mover** a linha do humano. A evidência que serve é a
  mediana de ações **TOTAIS da mesa** — medida em **360–377** por rodada, com razão total÷humano de
  **3,79–4,06 ≈ 4×**, que é o que **quatro assentos sem loop** produzem. ➕ E os zeros de `Error` cru
  e de teto de 30.000 ações valem para a mesa **inteira**, assentos de bot inclusive: uma partida
  travada bateria o teto, e **nenhuma bateu em 960**. ⚠️ Isto é evidência **contra o loop nas
  condições medidas**, não prova de impossibilidade — o `>` estrito continua sendo o que o impede.
- 🔴 **"Mutação verde = o dublê não produz o cenário" apareceu MAIS 2 vezes** (10ª e 11ª ocorrências
  catalogadas), e a 11ª é a mais reutilizável desta fatia: uma **auto-revisão do implementador**
  rodou uma mutação `>` → `>=` **não prescrita por ninguém** e achou **43/43 verdes** — a regra
  anti-loop que o spec chama de mais importante da fatia **não tinha um único teste mordendo**,
  porque todo ganho daquele ramo era negativo e os dois comparadores só divergem em **exatamente
  zero**. É a primeira desta família achada **pelo próprio implementador, sem revisor**. A 10ª: a
  Task 1 mudou por acidente a política de produção do bot para o `escudo-redondo` e **nenhum teste
  podia pegar**, porque os **cinco** dublês de mão do catálogo de teste declaravam `maoDireita`.
- ⚠️ **Um `AcaoInvalida` alcançável VIVEU ENTRE DUAS TASKS, e é fato de processo que vale registrar:**
  ...
  propagado por `avancarBots` = **400 na ação do HUMANO**, e a branch **não podia ser dev-servida nem
  soakada**. Confirmado **inalcançável pela suíte** (não era falha mascarada), declarado no ledger
  como primeira obrigação da task seguinte, e fechado por ela **no único ponto de emissão**. ➡️ A
  lição não é *"não separe as tasks"*: é que **a janela precisa ser NOMEADA**, senão alguém sobe o
  dev server no meio dela e persegue um fantasma.
- ⚠️ **A 16ª ocorrência do vício nº 1 saiu em TRÊS variantes nesta fatia, todas pegas em revisão:** um
  comentário de Zod afirmando *"o SLOT não viaja no fio"* **depois** de a mão começar a viajar; um
  comentário do bot dizendo que ele *"nunca produz `AcaoInvalida`"* na **mesma task** que tornou isso
  falso; e um comentário da tela dizendo que "Equipar" nasce *"nesta lista e não na da mão"*.
  🔑 **As três são a mesma família e o mesmo momento:** o comentário descrevia o presente **de antes
  do diff em que ele estava**.
- 📌 **O harness do soak tinha um defeito de CONTAGEM que inflava três denominadores, e a prova da
  correção é aritmética, não estatística:** `queimarCarta` também emite `desequipou` ao resolver a
  fila, então todo deslocamento roteado por queima era contado **duas vezes**. Em **todas as 12**
  rodadas, o contador de `mochilaEncolheu` deu **exatamente 2×** as aberturas (razão 2,000, doze
  vezes). Denominadores corrigidos **sem re-rodar**, por aritmética sobre os dados já coletados:
  `trocaDeSlot` **3.859** (não 5.053) · `mochilaEncolheu` **393** (não 786) · `perdeuAfinidade`
  **≈620** (não 874). **Os numeradores não mudam** (`queimarCarta` sempre resolve 1 por vez), e
  **nenhuma conclusão virou** — mas o N errado **sobreviveria ao harness apagado**, que é o que fazia
  disto um achado e não um detalhe.

### Sessão de 2026-08-08 (madrugada) — a esquiva muda de stat OUTRA VEZ, e nasce o teto

⚠️ **Esta sessão EMENDA a #105, que tem menos de um dia de vida.** A #105 fica escrita e **não é
apagada**: ela acertou o diagnóstico (*"o defensor precisa ter um stat na esquiva"*) e **errou o
stat**. O que a derrubou foi aritmética conferida contra o catálogo de produção, não gosto.

| # | Decisão | Por quê |
|---|---|---|
| 106 | 🎲 **A ESQUIVA É PELA AGILIDADE, não pela habilidade. EMENDA a #105.** A condição passa a ser `rolagem do defensor ≤ AGILIDADE dele`. ⚠️ **O ataque não muda** (`rolagem ≤ habilidade do atacante`), e a rolagem do atacante **continua viajando** — mas agora só para o **Golpe Certeiro** (`≤ 2`) do Ladino. ⬜ **NÃO CONSTRUÍDA** | 🔑 **O problema nunca foi o dado — era UM STAT COM DOIS TRABALHOS.** Com a habilidade acertando **e** esquivando, os dois efeitos **se multiplicam**: hab. 9 contra hab. 6 conecta **3× mais** (37,5% × 12,5%). Com a agilidade esquivando, cada stat fica com **um** trabalho — habilidade acerta · agilidade esquiva · força dana · vida dura — e o composto **some**. ✅ E o spec original já previa: *"candidato a refinamento futuro: dar papel à Agilidade"* — ela era o stat mais magro do jogo (só iniciativa). 💰 **Custo aceito:** o `agilidade −1` de Cota de Malha, Placa do Clã e Montante deixa de ser decorativo, e o `PISO = 1` passa a valer (agilidade 1 = 8,3% de esquiva). 💰 **Composto MENOR aceito de propósito:** a agilidade continua decidindo **iniciativa**, então ela faz duas coisas — mas iniciativa é **uma vez por combate** e esquiva é **toda troca**, e o Pedro recusou explicitamente a opção de tirar a iniciativa dela |
| 107 | 🎚️ **NASCE O TETO DE STAT, composto por RAÇA + CLASSE, SOMADO — e ele vale só para HABILIDADE e AGILIDADE.** Cada raça e cada classe declara a sua contribuição ao teto, e as duas **somam** sobre um teto-base. 🔴 **Teto máximo global 9**, mesmo para a melhor combinação (declarado pelo Pedro para os **dois** stats). ⬜ **Desenho não fechado** — ver a pergunta **21** do §18 | 🔑 **Duas razões independentes, e as duas são necessárias.** **(1) É o FREIO da #106:** `esquiva = agilidade/12` é uma curva **convexa** — vida efetiva = `vida × 12/(12 − agilidade)`, ou seja ×1,7 com agi 5, ×3 com 8, ×4 com 9 e **infinito com 12**. Sem teto, *"empilhar só item ágil"* vira a estratégia dominante e agilidade 12 é **imortalidade** — e **agilidade 9 já é alcançável no catálogo de hoje** (Aquático Ladino com Botas de Maré). **(2) É a MÁQUINA DE ADAPTAÇÃO que o Pedro pediu** (*"não quero as mesmas decisões todos os jogos… ele tem um Orc Ladino, quais itens ele tem que buscar?"*): raça e classe são **cartas que se compra, não se escolhe**, então o teto é sorteado por partida, e o mesmo item vale coisas diferentes para pessoas diferentes. 🔑 **Por que só habilidade e agilidade:** são as duas que rolam contra o d12 e por isso **quebram em 12** (acerta sempre / esquiva sempre). **Força e vida são linha reta e não precisam de teto** — o perfil delas continua saindo de **modificador**, que é o que a classe já dá hoje. Uma manopla por stat, cada uma no lugar por um motivo. 💰 **Custo declarado: revoga o §5** (*"raça = uma passiva, **NÃO stats**"*), cuja justificativa escrita era manter o Humano jogável — teto é stat, ainda que negativo, e essa justificativa precisa ser **reexaminada, não contornada** |
| 108 | 📌 **REGISTRO DE INTEGRIDADE: a #105 e o `docs/game-design/expansoes-pos-mvp.md` descreviam REGRAS DIFERENTES, escritos no mesmo dia.** O doc pós-MVP (`§Esquiva`) escreveu `esquiva se rolagem ≤ min(habilidade do defensor, rolagem do atacante)`; a #105 escreveu `rolagem ≤ habilidade DELE`, **sem o `min`**, e calculou o *"~29% → 50%"* em cima da versão **sem** `min`. **São regras com comportamentos opostos** | 🔑 **A divergência é instrutiva e por isso fica registrada em vez de ser corrigida em silêncio:** as duas versões não diferem em número, diferem em **FORMA**. A versão com `min` é **côncava e satura** — a agilidade/habilidade só vale até a habilidade de quem ataca, e como os monstros têm habilidade **2–4**, ela seria **INERTE** contra 3 dos 5 (esquiva idêntica a hoje) e mudaria no máximo **2 pontos percentuais** no pior caso realista do catálogo. A versão sem `min` é **convexa** e precisa de teto. ➡️ **A #106 escolheu a forma sem `min`, com teto explícito** — a saturação vem do teto, que é legível e desenhável, e não da rolagem do adversário, que traz de volta a inversão que a #105 existia para consertar |

#### 📐 A aritmética conferida nesta sessão — premissas declaradas, **NÃO medição**

Tudo abaixo sai das fórmulas (`acerta se rolagem ≤ habilidade`, `dano = level + forca`) contra o
catálogo de produção lido em `packages/cartas/src/` e a `BASE` de `packages/personagem/src/montar.ts`
(`forca 3 · vida 10 · habilidade 6 · agilidade 5 · level 1`). **Nenhum soak rodou.**

- 🔴 **A #105 afirmou que *"o combate fica bem mais longo"*, e isso está ERRADO para o jogo que
  existe.** O *"~29% → 50%"* é calculado com **habilidade 6 dos dois lados** — um espelho que a mesa
  não tem. Os monstros têm habilidade **2–4**, então qualquer regra que ponha o stat no **defensor**
  tira deles uma proteção que tinham **de graça** (atacante desajeitado só acerta rolando baixo, e
  rolagem baixa era **difícil de esquivar**). Jogador nu × Goblin, ataques até matar:
  **hoje 14,1 × 13,7** (moeda ao ar) → **#105 12 × 24** → **#106 15 × 20,6**. ➡️ **As duas versões
  deixam o jogo MAIS FÁCIL**, a #106 menos que a #105. **O catálogo de monstros vai precisar de
  conta — pergunta 24 do §18.**
- 🔴 **A afirmação *"retorno decrescente forte"* do `expansoes-pos-mvp.md` (§Esquiva, item 3) está
  MAL ROTULADA, e ela inverte o conselho de desenho.** O `(12 − N)/144` é o valor de um ponto
  **ofensivo contra um defensor de habilidade N** — afirmação sobre o **alvo**, não sobre quem
  compra. Somando os dois lados num espelho em N, o ofensivo `(12−N)/144` **cai** e o defensivo
  `N/144` **sobe**: o total é `12/144` = **8,33% por ponto, CONSTANTE**. ➡️ **A habilidade nunca se
  auto-limitou.** O 8º ponto vale o que o 1º, e por isso o teto da #107 é **o único freio**, não uma
  cobrança em cima de uma curva que já existia.
- 🔑 **A complementaridade habilidade × força JÁ EXISTE e é invisível.** Dano por troca =
  `(habilidade/12) × (level + força)` é **multiplicação**: o ponto de virada é exatamente
  **`habilidade = level + força`**. Na base (hab 6, dano 4) **força é a melhor compra** (0,50 contra
  0,33); depois de duas Espadas Curtas (dano 8) **habilidade passa a ser** (0,67 contra 0,50).
  ➡️ *"Focar em só item hábil"* **já é ruim hoje** — o defeito não é a estratégia existir, é o jogo
  **não contar** que ela é ruim (a tela não mostra, e `vestirOuGuardar` **soma** os modificadores em
  vez de multiplicar).
- 🔑 **HOJE OS CINCO MONSTROS SÃO DEFENSIVAMENTE IDÊNTICOS, e a #106 conserta isso de graça.** Como
  a esquiva depende da rolagem de **quem ataca**, todos esquivam de um jogador de habilidade 6 na
  **mesma taxa: 29,2%** — a `agilidade` escrita na carta deles **não faz nada**. Com a #106 elas
  passam a valer: **Ogro 17% · Rato 25% · Goblin e Carniçal 33% · Lobo Sombrio 58%**. ➡️ Sem escrever
  **uma carta nova**, os monstros passam a pedir coisas diferentes (contra o Lobo, habilidade é ouro
  e força quase não conecta; contra o Ogro, o inverso) — e o jogador **vê a carta antes** de decidir
  procurar encrenca. **É a variedade mais barata que o jogo tem disponível.**
- ⚠️ **O mesmo número de teto NÃO vale o mesmo nos dois stats, e isso é aritmética, não opinião.**
  Habilidade é **linha reta** (5→9 = 42%→75% de acerto = **1,8×**); agilidade é **bola de neve**
  (3→9 = ×1,3→×4,0 de vida efetiva = **3,0×**). Com teto **9 nos dois**, quem empilha agilidade sai
  quase **duas vezes** mais premiado. **Recomendação registrada e NÃO respondida pelo Pedro:**
  habilidade máx. **9**, agilidade máx. **7** (×2,4, o equivalente justo). ➡️ Se o 9 ficar nos dois
  por simetria de leitura, a agilidade **tem que custar mais caro em vida** — a conta dá vida ~6
  contra os ~18 de um Orc Guerreiro, o que transforma a build ágil numa **build de cara-ou-coroa**
  (um golpe que passa mata). Arquétipo legítimo; tem que ser **escolha, não acidente**.

#### 🔴 O ESCOPO MUDOU — a #105 pegava carona nas Maldições, e a #106 já não cabe lá

A #105 era **uma linha** em `packages/motor/src/ataque.ts`, e por isso o Pedro aceitou embarcá-la na
fatia **Maldições / Bad Stuff**. A #106 + #107 são: trocar o stat da esquiva **mais** um campo de
...
| # | Decisão | Por quê |
|---|---|---|
| 110 | 🧩 **O BLOCO 2 É DECOMPOSTO EM QUATRO FATIAS, nesta ordem: 2a Bad Stuff + evacuação · 2b consumíveis (`instantâneo`) · 2c maldição no `vasculhar` · 2d maldição na mão (mira + concorrência).** O §3.1 escrevia o bloco 2 como **uma** fatia | 🔴 **Como escrito, o bloco 2 são SEIS eixos independentes:** (1) família de carta de Portas **com efeito** — hoje **nenhuma carta do jogo tem verbo próprio**, monstro/raça/classe são dado puro; (2) **mira** (ação com alvo, evento de duas pontas, seletor na tela); (3) **concorrência** na `jogar` de qualquer jogador (#47), 🔴 **bloqueada pela pergunta 16 do §18, que segue aberta**; (4) Bad Stuff ao perder combate (hoje só `derrotas + 1`, `mesa.ts:1312`); (5) **evacuação**, que interage com a pendência de queima (#59) e com `combatenteDe`; (6) **consumíveis ≥50%** (#40), família nova no baralho de **Itens**. 🔑 **O argumento que decidiu é de MEDIÇÃO, não de gosto:** esta é a fatia da **economia**, e a economia tem **um** número — o baralho de Tesouros esgota em **480/480 partidas**. Os eixos **5** e **6** são **os dois caminhos de volta** que o jogo vai ganhar; juntos, **nenhum número diria qual consertou**. É a **#51** com seis variáveis em vez de duas. 💡 **E um achado que destravou metade da #40:** o **`instantâneo` é construível AGORA** — a #44 declara *"custo de ritmo ZERO, ninguém novo é consultado"*, e o código confirma (`proximoPasso` já para duas vezes por round esperando o lutador, e `EstadoPartida.combate` já persiste entre requisições). ➡️ Ele **não** depende do bloco 5; a `carta de combate` depende (é janela A, #43). ⚠️ **Por que o 2a primeiro:** a **#46** diz por escrito que a razão principal do bloco 2 é o conserto da economia e que a evacuação é *"o maior caminho de volta de cartas do jogo inteiro"* — é o maior efeito com os menos eixos novos. **Por que o 2d por último:** é o único **bloqueado por decisão em aberto** |
| 113 | ✅ **A PATENTE NÃO RESETA. A #111 é EMENDADA no mesmo dia, e o §10 fica intacto.** Morte = evacuação **total** (mão, equipamentos, mochila), **mantendo a patente**, e o jogador recomeça **no turno seguinte dele** comprando **4 Portas + 4 Tesouros**. ⬜ **NÃO CONSTRUÍDA** | Decidido pelo Pedro em 2026-08-08, **depois de ler a objeção**: *"não precisa resetar a patente então"*. 🔑 **Esta entrada existe para registrar o MECANISMO, não só o resultado:** a #111 tentou reverter uma linha do §10 que **já era ela mesma uma correção** de um desenho anterior — e o parágrafo *"Por que mantém a patente"* estava lá, escrito, com o argumento pronto (*"voltar à patente 1 no minuto 30 é estar matematicamente eliminado… na prática ele abandona, e a mesa de 4 vira 3"*). ➡️ **A objeção foi apresentada UMA VEZ, sem insistência, e o dono reverteu sozinho.** É o caso em que a justificativa escrita no bible **fez o trabalho para o qual ela existe** — e é a razão de este projeto escrever o *porquê* junto da decisão em vez de só a decisão. ✅ **O que sobra da #111 e FICA:** o *"recomeça comprando mão nova"* do §10 ganhou **número** (4+4, as constantes `MAO_INICIAL_PADRAO`/`MAO_INICIAL_TESOUROS` de hoje) e **momento** (o turno seguinte do próprio jogador). ⬜ **Continua aberto:** a evacuação leva `emJogo.raca` e `emJogo.classe`? |
| 111 | 🔴 **EMENDADA PELA #113 NO MESMO DIA — não vale. Fica registrada porque o mecanismo importa.** 💀 **MORTE = EVACUAÇÃO TOTAL **MAIS RESET DE PATENTE**, e o jogador recomeça NO PRÓPRIO TURNO SEGUINTE comprando 4 Portas + 4 Tesouros.** ⬜ **NUNCA CONSTRUÍDA** | Decidido pelo Pedro em 2026-08-08: *"morte o player perde tudo, reseta a patente, recomeça o jogo dele no seu próximo turno (cava 4 portas, 4 tesouros e recomeçar o game)"*. 🔴 **ISTO REVERTE O §10, E O §10 JÁ TINHA SIDO CORRIGIDO NESTA DIREÇÃO UMA VEZ** — ele diz *"mantém a patente"* e marca essa frase como *"correção de uma versão anterior deste doc, que mandava pro nível 1"*. ➡️ **O desenho novo é o desenho VELHO voltando**, e a objeção escrita continua de pé, **não respondida**: *"numa corrida ranqueada de 45 min com classificação, voltar à patente 1 no minuto 30 é estar **matematicamente eliminado** — o jogador não tem mais nada a defender, e o **kingmaking** que a classificação resolveu volta. Na prática ele abandona, e a mesa de 4 vira 3."* ⚠️ **A objeção foi levantada UMA VEZ e o Pedro NÃO a respondeu** (a sessão terminou logo depois) — ela fica anexada aqui e no §10 em vez de o texto ser consertado em silêncio numa das duas direções, que é o precedente da **#67**. 📌 **O que a decisão FECHA de útil, independentemente disso:** o *"recomeça comprando mão nova"* do §10 ganha número (**4+4**, a `MAO_INICIAL_PADRAO`/`MAO_INICIAL_TESOUROS` de hoje) e **momento** (o turno seguinte do próprio jogador, não imediatamente) — os dois eram lacunas reais. ⬜ **E fica aberto o que a evacuação faz com `emJogo.raca` e `emJogo.classe`:** o §10 lista **três** zonas (mão, equipamentos, mochila) e a especialização **não está nelas**. Ninguém decidiu |
| 112 | 🐲 **O BAD STUFF É POR MONSTRO, não uma regra uniforme da derrota** — decisão do Pedro em 2026-08-08, **contra a minha recomendação**. ⬜ **NÃO CONSTRUÍDA** | Ele confirmou a leitura literal do §10 (*"Bad Stuff (efeito da carta)"*). ⚠️ **Custo declarado e ACEITO, que era o meu argumento:** isso **puxa o eixo 1 (a máquina de efeito) para dentro do 2a**, que é exatamente o eixo que a #110 tinha empurrado para o 2c — `MonstroCarta` ganha um campo e com ele nasce um **vocabulário de efeito** (*perde N itens? perde a mochila? evacua?*), que é a primeira vez que uma carta deste jogo **faz** algo. 🔴 **E suja a única medição da fatia:** com Bad Stuff variando por monstro, quanto de carta volta ao baralho passa a depender de **quais monstros foram lutados**, então o número do esgotamento deixa de ser limpo. 📌 **Minha recomendação registrada e recusada era (c):** uniforme agora, por monstro quando a máquina de efeito existisse — (b) é **especialização** de (a), não reescrita, então o caminho existia. **O Pedro escolheu (b) e é decisão dele** |
| 114 | 🐲 **O VOCABULÁRIO DO BAD STUFF: dois verbos, `evacuacao` e `perdeSlot(família de item)`.** Dado na carta (`MonstroCarta.badStuff`, **obrigatório**) + interpretador em `partida`. `perdeSlot` nomeia uma **família de item** (`SlotDeItem`: `capacete · armadura · mao · pes`), e `mao` limpa **as duas vagas**. O item arrancado vai **direto ao cemitério de Tesouros**. ⬜ **NÃO CONSTRUÍDA** — spec em `docs/superpowers/specs/2026-08-08-bad-stuff-e-evacuacao-design.md` | 🔑 **"Dado + interpretador" NÃO foi escolha — a direção de dependência decidiu.** As passivas podem ser código dentro de `cartas` porque só tocam o `Combatente`, que é um **valor**; o Bad Stuff toca **mão, mochila e slots**, que são zonas de `partida`, e `cartas ← personagem ← partida` significa que uma função em `cartas` **não enxerga** as zonas. ✅ De quebra, `MonstroCarta` continua **dado puro** e dispensa projeção `Resumo` (§4). 🔑 **Nomear a FAMÍLIA e não o encaixe físico mata duas coisas de uma vez:** (1) depois da **#98** as duas mãos são vagas equivalentes, então *"por que a direita?"* não tem resposta; (2) 🔴 **a arma de duas mãos põe a MESMA instância nos dois encaixes e `itensEquipados` deduplica por id** — limpar só um deixaria o Montante **equipado dando stats cheios**, e o Bad Stuff faria literalmente nada contra quem o usa. ⚠️ **Passaria verde em qualquer teste sem Montante no dublê** — é a família *"mutação verde = o dublê não produz o cenário"*, 11 ocorrências, cuja causa raiz nunca foi desatenção. 🔴 **E o destino ser o CEMITÉRIO é assimetria deliberada com `destinoDoDesequipado`** (que prefere a mochila): trocar de equipamento é **sua escolha**, o Bad Stuff é o monstro **tomando**. Se fosse à mochila, o item voltaria ao corpo na fase `jogar` do mesmo turno (a punição vira nada) **e devolveria zero carta ao baralho** (a economia vira nada) — as duas razões da fatia esvaziadas de uma vez. ✅ Sem mochila no caminho, `perdeSlot` **nunca abre pendência de queima**. 🎚️ **Só o monstro de 3 tesouros (o Ogro) evacua** — 1 de 5. Três razões: evacuação a uma por partida vira rotina e deixa de assustar; escala limpo com o prêmio (hoje perder **não custa nada**, e o dial do `procurarEncrenca` não morde); e se ela sozinha consertasse a economia o **2b ficaria sem trabalho**, sendo a **#40** regra ESTRUTURAL. 📊 **Consequência honesta, declarada no spec:** com ~15 Tesouros de volta por partida num baralho de 48, esta fatia **provavelmente NÃO conserta a economia sozinha** — é informação, não falha. ⚠️ **Derivado, não medido** |
| 115 | 🧬 **A EVACUAÇÃO NÃO LEVA `emJogo.raca` NEM `emJogo.classe`.** Leva mão + mochila + os cinco encaixes de equipamento; ficam a patente (#113), a especialização e o contador de derrotas. ⬜ **NÃO CONSTRUÍDA** | Três razões, e a segunda é a mais forte. **(1)** É a lista **literal** do §10, que nomeia **três** zonas — *mão, equipamentos, mochila* — e a especialização não está nelas. **(2)** 🔑 **A #38 já tinha resolvido isto sem saber:** a carta de raça é um *"artefato de transformação, **consumido no uso**"*; o pergaminho **já foi gasto**, e o que está em jogo é a **raça resultante**, não uma carta que se carrega. Mandá-la ao cemitério seria **desconsumir** algo consumido. **(3)** É o que faz o *"momento de virada"* do §14 existir: reerguer-se do zero **como Guerreiro** é uma história; virar um Aprendiz nu é só ter perdido. ⚠️ **Custo declarado:** quem evacua sai com vantagem sobre quem nasceu Aprendiz — mantém os modificadores da classe de graça. Deliberado, pelo mesmo motivo da #113: a evacuação já é durísssima e precisa deixar de onde reconstruir. 📌 **E economicamente é neutro:** raça e classe são cartas de **Portas**, e pela **#62** o baralho de Portas **nunca acaba** — o número que importa é o de Tesouros. Isto foi decisão de jogo puro, sem contrapartida de baralho |
| 116 | 🔁 **QUEM VOLTA DE UMA EVACUAÇÃO ENTRA EM `recompor`, com 4 Portas + 4 Tesouros — mesmo estando ACIMA do limite de mão.** ⬜ **NÃO CONSTRUÍDA** | 🔴 **A regra nasceu de um achado no encanamento, não de gosto.** Quem evacua **mantém a raça** (#115), logo o limite dele é **7**, e `4 + 4 = 8`. Se a fase saísse de `faseDoTurnoDe`, ele voltaria direto em **`descartar`**, onde a única ação legal é `entregarCarta` — **caridade** —, e `entregarCarta` termina em `encerrarTurno`, que então o vê dentro do limite e **passa a vez**: ➡️ *ele evacuaria, esperaria uma rodada inteira, voltaria, **doaria uma carta a um rival e perderia o turno de novo***. ✅ **A regra é o §11 valendo:** *"a mesa nasce exatamente no teto; quem devolve a folga é **equipar**, não a caridade"* — quem volta está na mesma situação de quem nasce, só que **com raça**, e é a raça que tira o 8º espaço. Em `recompor` ele tem **três** saídas (equipar, guardar, jogar raça/classe) com 8 cartas novas; se desperdiçar as três, cai em `descartar` no fim do turno, que é a regra normal de todo mundo. 📌 **Por que 4 Portas e não 3** — argumento do **Pedro**, que substituiu uma recomendação minha de cortar uma Porta: *"pode ser que ele queira trocar de raça, venha uma maldição que ele queira usar"*. As Portas não são "encher mão"; cortar uma corta a **variedade** que faz o recomeço ser jogada e não sorteio. 🔴 **E a ORDEM é load-bearing: comprar ANTES de calcular a fase.** Calcular antes daria a fase a um jogador de mão vazia, que se auto-pularia — é **exatamente** o bug do Plano 4a (`equiparCarta` passando a `entrarOuPular` o jogador de antes da segunda mutação), mesma família e mesmo arquivo. **Vira teste com mutação, não comentário** |
| 117 | ⚰️ **A PILHAGEM DO CADÁVER VIRA FATIA PRÓPRIA — a `2a-bis`, logo depois da 2a.** Desenho do Pedro: ao morrer, **todas as cartas do jogador ficam ABERTAS**; cada um dos outros três escolhe **uma** no início do próprio turno; o que sobra é descartado; e então o morto volta com 4+4. ⬜ **NÃO CONSTRUÍDA, e o desenho NÃO está fechado** | ✅ **A mecânica é boa e não está sendo recusada** — ela transforma a morte de punição solo em **evento de mesa**, e é *"a primeira mecânica que toca outro jogador"*, que é a **justificativa (1) da #46** para o bloco 2 existir. 🔴 **O que a tira da 2a é o TAMANHO, contado:** ela traz **zona pública nova** (os despojos + quem já pegou), **verbo novo** (`pilharCadaver`), **pendência nova**, ⚠️ **uma pendência que ATRAVESSA TURNOS** (3 jogadores, 3 turnos — nenhuma pendência do jogo faz isso hoje), a **primeira quebra de sigilo** (a mão do morto vira pública) e política de bot. 🔑 **A evidência decisiva é o precedente desta base:** a fatia **`escolha do descarte`** foi *"abrir **uma** pendência com **um** verbo, dentro do mesmo turno"* — e foram **8 tasks e 597 testes**. A proposta é aquilo **mais** cinco coisas. ⚠️ **E ela muda o número que a 2a existe para medir:** desviaria ~3 cartas por morte das que iriam ao baralho (~24% do caminho de volta), enquanto acrescenta 6 eixos — é a **#51** outra vez. ⬜ **Pergunta que já nasce aberta e o Pedro não respondeu: e se DOIS jogadores morrerem antes de os despojos acabarem?** Duas pilhas ao mesmo tempo? ➡️ **Como fatia própria ela é candidata a trocar de lugar com a 2c/2d na ordem** — pode vir antes da maldição |
...
| 118 | 🛡️ **O BAD STUFF É INCONDICIONAL NA DERROTA — a escapatória vem de CARTA, nunca de um monstro sem punição.** *"Você sempre toma a badStuff a não ser que ele tenha alguma carta que diga o contrário."* ⬜ **NÃO CONSTRUÍDA** — e a carta que cancela é **fatia futura**, fora da 2a | Decidido pelo Pedro em 2026-08-09, durante a **revisão do spec da 2a**. 🔑 **A decisão nasceu de uma pergunta de MEDIÇÃO e respondeu pelo lado do JOGO, que é o lado certo.** O §8 do spec prometia medir *"Bad Stuff DESLIGADO × LIGADO"*, e a revisão mostrou que **não existe desligado**: o `badStuff` é obrigatório e os dois verbos têm efeito. A saída óbvia era um terceiro variante `{ tipo: 'nenhum' }` — e ela **está recusada por esta decisão**, porque reintroduziria explicitamente o que o campo obrigatório existe para impedir (monstro novo nascer sem punição em silêncio, o modo de falha da **#54**). ➡️ **A escapatória é conteúdo, não ausência:** ela chega como carta que o jogador joga, no eixo dos consumíveis da **2b**/**#40**, e por isso é a mesma família da mira das Maldições — algo que se **usa**, não algo que o monstro **deixa** de fazer. ✅ **Consequência imediata na 2a:** o controle da medição passa a ser **o Ogro em duas versões** (`perdeSlot` × `evacuacao`), mesmo build e mesma sessão — que mede a **margem da evacuação**, não o valor do Bad Stuff contra zero, e isso fica declarado. 📌 O Pedro nomeou o contexto em que a carta faz sentido: *"hoje tem poucos itens e poucas cartas, mas terão mais raças, classes, maldições"* |
| 119 | 🖥️ **O BAD STUFF É PÚBLICO E FICA ESCRITO NA CARTA DO MONSTRO.** O jogador lê o que aquele monstro faz com ele **antes** de escolher a luta. ⬜ **NÃO CONSTRUÍDA** — entra na **2a**, spec §5.3 | Requisito do Pedro em 2026-08-09: *"o texto do badStuff tem que ir para o front, na carta do monstro tem que ter escrito qual é a coisa ruim que ele faz ao derrotar um player"*. ✅ **Coerente com o que a carta de monstro JÁ É:** o `Catalogo.monstros` publica `MonstroCarta` **inteira**, sem projeção `Resumo`, e o docstring diz por quê — *"a carta é revelada com a face para cima"* e os stats *"são informação pública"*. O Bad Stuff é da mesma natureza. 🔑 **Consequência de desenho, não só de UI:** `procurarEncrenca` é uma **escolha**, e o Bad Stuff é o **lado do risco** dela — sem o texto na tela, o jogador escolhe a luta às cegas e o dial *"só o Ogro evacua"* (#114) não tem como ser lido por ninguém. 🔴 **E ela fecha, ANTES de acontecer, a 7ª ocorrência de "publicado e nunca renderizado"**: como a carta inteira já viaja, o `badStuff` chega ao cliente **de graça** no instante em que entra no `MonstroCarta` — dado que viaja e ninguém desenha é o padrão que já escondeu a tese de um plano três vezes neste projeto (`combatente`, `tesourosNoMonte` ×2, `ehBot`, `mochila`, `cartasNoCemiterio`). **É a primeira vez que essa família é EVITADA em vez de descoberta.** 📌 Duas superfícies decididas: o **painel do combate** (o monstro fica à vista a luta inteira) e a **carta de monstro na mão** (onde mora o botão "Procurar encrenca"). **Fora:** a espiada da Presciência, que hoje nem nomeia o monstro (*"você pressente um monstro adiante"*), e o log, onde repetir a punição a cada porta seria ruído |
| 120 | 🎲 **UM MONSTRO PODE TER MAIS DE UM EFEITO DE BAD STUFF — `badStuff` é LISTA, não efeito único.** Hoje **todos têm exatamente um**. ⬜ **NÃO CONSTRUÍDA** | Decidido pelo Pedro em 2026-08-09: *"um monstro pode ter uma badStuff que faz diversas coisas, mas isso é pensando nos designs de monstros futuros"*. ✅ É *"arquiteta para o futuro, constrói para o presente"* com custo de hoje igual a **um laço**. 🔑 **O precedente é exato e é deste repo: o Plano A da `classe como carta`**, que levou o motor de `passiva` (uma) para `passivas: readonly EstadoPassiva[]` (N) **antes de qualquer carta ter duas** — e a fatia seguinte provou que foi certo. 🔴 **E ele traz junto a armadilha do precedente:** com toda lista de produção em tamanho 1, a mutação `efeitos.slice(0, 1)` fica **VERDE**, porque **nenhuma carta percorre o laço**. É *"ramo sem visitante"* somado a *"mutação verde = o dublê não produz o cenário"* (11 ocorrências), e o conserto é o mesmo das 11: **dublê de monstro com DOIS efeitos**, como `composicao.test.ts` precisou de dublês não comutativos para a ordem das passivas. ⚠️ **O tipo deixa um buraco que o teste tem que fechar:** *"obrigatório"* não impede `[]`, então o catálogo cobra `badStuff.length > 0` **por monstro** (e não por `.find`, que é a #54 por outra porta). 📌 **Absorção declarada:** `evacuacao` absorve o que vier depois — não sobra nada para `perdeSlot` arrancar — e as duas ordens convergem no mesmo estado final. **É esperado, e vira teste em vez de comentário** |

### Sessão de 2026-08-09 — a fatia **2a** está CONSTRUÍDA, e a economia continua aberta

✅ **As decisões #112, #114, #115, #116, #118, #119 e #120 saem de `⬜ NÃO CONSTRUÍDA` para
CONSTRUÍDA** (branch `feat/bad-stuff-e-evacuacao`, 732 testes verdes, typecheck 7/7).
🔴 **Gate ocular do Pedro PENDENTE — nenhum item conferido.** Relato completo em
[`docs/historico/2026-08-09-bad-stuff-e-evacuacao.md`](../historico/2026-08-09-bad-stuff-e-evacuacao.md).

⚠️ **As #121 e #122 são REGRA DE JOGO nascida na EXECUÇÃO, e as duas nasceram de BUG achado pelo
soak** — não de desenho. As #123–#126 são **registros de MEDIÇÃO**.

| # | Decisão | Por quê |
|---|---|---|
| 121 | 🔁 **O RECOMEÇO ANEXA À MÃO, NUNCA A SUBSTITUI — e por isso quem volta de uma evacuação pode voltar com MAIS de 8 cartas.** ✅ **CONSTRUÍDA** | 🔴 **Nasceu de um BUG Critical que o soak achou e 730 testes não** (medido: **35 de 240 partidas, 14,6%, 81 cartas distintas perdidas**). `comprarMaoInicial` **substituía** a mão por inteiro. 🔑 **A cadeia é consequência direta da #113:** quem evacua **mantém a patente**, logo continua alvo **legítimo** de caridade enquanto espera a vez voltar — `entregarCarta`/`destinoDaCaridade` filtram por patente e **nunca checam `evacuado`** —, e a carta doada nesse intervalo **sumia de todas as zonas**, sem `descartarNoBaralhoCerto`, sem evento, sem log. ✅ **Voltar acima do teto é coerente com a #116**, que já o faz voltar com 8 de propósito: em `recompor` ele tem três saídas, e só cai em `descartar` se desperdiçar as três — a regra normal de todo mundo. 🔑 **Quem pegou foi o CENSO DE CONSERVAÇÃO id-a-id depois de cada ação, e é a primeira vez nesta base que ele pega um bug** — nenhum teste alcançava o cenário |
| 122 | 🔴 **O RECOMEÇO TRATA O BARALHO DE TESOUROS ESGOTADO COM GRAÇA — leva o que houver e emite `tesouroEsgotado`. PORTAS continua sem guard, de propósito.** ✅ **CONSTRUÍDA** | 🔴 **Nasceu de um `Error` cru = 500 EM PRODUÇÃO** que o soak achou (**1 em 240 partidas**). O laço de Tesouros de `comprarMaoInicial` chamava `tirarDoTopo` **sem checar esgotamento**, enquanto `sacarTesouros` — o **outro consumidor do mesmo baralho** — trata graciosamente desde a **#28**. ⚠️ **A conjunção não é hipotética:** com **91,7%–97,9% das partidas esgotando** (#125), *"a vez de um evacuado cai quando Tesouros está seco"* é caminho real. ✅ **A assimetria com Portas é a #62 valendo:** faltar Porta é **invariante NOSSA quebrada** (500), não pedido inválido — o `Error` cru ali **fica**. 🔑 **A lição de forma:** dois consumidores do mesmo baralho com políticas de esgotamento **diferentes** é o defeito; o conserto foi **espelhar o guard que já existia**, não inventar um terceiro comportamento |
| 123 | 📊 **A MARGEM DA EVACUAÇÃO, MEDIDA: +13,57 cartas por partida aos cemitérios** (Portas **+1.082**, Tesouros **+2.175**, em 240 partidas por braço) | **Controle interno**, mesmo build e mesma sessão, **UMA variável — o `badStuff` do Ogro**: braço **A** `perdeSlot('armadura')` × braço **B** `evacuacao`. 🔴 **O que isto mede, dito com precisão: a MARGEM da evacuação sobre uma punição LEVE — não o valor do Bad Stuff contra zero.** O braço A **também** devolve carta (554 Tesouros, 2,31/partida contra 15,88/partida do B); o valor absoluto da fatia fica **NÃO MEDIDO** e declarado como tal. ⚠️ **O zero de Portas no braço A é ESTRUTURAL, não empírico:** `perdeSlot` nunca alcança a mão — só a `evacuacao` toca as três zonas do §10, e é essa a diferença que a fatia promete. ⚠️ **Ressalva de denominador:** as derrotas diferem entre os braços (**795 × 778**), porque a evacuação muda a dinâmica dali em diante; a comparação **por partida** (N=240 fixo nos dois) é a que não sofre esse viés, e **a comparação por derrota seria enganosa**. ✅ **Este controle é IMUNE à ressalva-mãe** que a `empunhadura dupla` não conseguiu contornar: os dois braços saem da mesma sessão, então **não há comparação entre fatias para licenciar** |
| 124 | 📊 **EVACUAÇÕES: 0,364 por jogador por partida** (349 em 240 partidas; 1,454 por partida de mesa). **E 32,6%–33,3% dos `perdeSlot` acertam encaixe VAZIO** | ✅ **A previsão do spec era ~0,3 por jogador, DECLARADAMENTE enviesada para baixo** (a derivação assume taxa de derrota uniforme entre os 5 monstros, e o Ogro é o mais duro — a derrota **concentra nele**). **O medido fica ACIMA de 0,3, na direção que o spec nomeou**, e o sinal **se repetiu em sessão independente** (a rodada anterior deu 0,352). 🔑 **A identidade fecha exata e prova a atribuição do §3.2 do spec:** `778 derrotas − 349 evacuações = 429 perdeuEquipamento` — **toda** derrota produz **ou** um **ou** o outro, nunca os dois nem nenhum. 🔴 **Braço A: ZERO evacuações, ESTRUTURAL** — nenhum monstro dele tem o verbo. ⚠️ **O "escapa de graça" é 1 em 3, não exceção** — e é exatamente por isso que o evento é emitido **mesmo com `cartas: []`** (#28): sem a linha, *"o Goblin mirou o seu capacete e você não usa capacete"* seria indistinguível de silêncio. ⚠️ O gradiente por assento (87·83·93·86) **não é interpretado**: sem N nem controle, e o formato mudou entre as duas rodadas |
| 125 | 🔴 **A EVACUAÇÃO NÃO CONSERTA A ECONOMIA SOZINHA — o baralho de Tesouros ainda esgota em 91,7% das partidas (contra 97,9% do braço de controle).** A **#40** segue sendo a resposta ESTRUTURAL, e os consumíveis da **2b** seguem com trabalho | ✅ **O spec PREVIU isto por escrito** (*"com ~15 Tesouros de volta por partida num baralho de 48, esta fatia provavelmente NÃO conserta a economia sozinha"*) — **é informação, não falha**: a fatia entrega o caminho de volta e **mede quanto ele vale**. 📊 Fração de ações com monte **E** cemitério de Tesouros vazios: **16,36% (A) × 13,78% (B)**, N=240 partidas cada. ⚠️ **SÃO DUAS COMPARAÇÕES DIFERENTES, e colapsá-las inverte a leitura. (i) A × B na mesma rodada está LICENCIADA** — mesmo build, mesma sessão, uma variável —, e a diferença (**−6,2pp** nas partidas) **é atribuível ao `badStuff` do Ogro**; o que ela **não** diz é o **mecanismo**, porque as **795 × 778** derrotas diferem entre os braços e a atribuição entre *"aconteceram menos derrotas"* e *"cada derrota devolveu mais"* fica **aberta**. **(ii) Rodada × rodada dentro do MESMO braço é RUÍDO, e serve de RÉGUA:** o braço A (que o bug da #121 nunca alcançava) **se moveu +0,81pp / +1,2pp entre duas rodadas com o MESMO código**, e o braço B se moveu **dentro dessa ordem e sem direção consistente** — ➡️ **o conserto do bug NÃO moveu o braço B de forma distinguível**, o que é compatível com o bug ter sido real e pequeno demais para pegada neste N. 🔑 **A diferença A × B é ~5× a régua**, e é isso que a separa de acaso. ⚠️ **Não é intervalo de confiança:** um par de sessões, régua de duas observações. 🔑 **A leitura de roteiro é a que importa:** a #114 já dizia que, se a evacuação sozinha consertasse a economia, o **2b ficaria sem trabalho**. Ela não consertou — **a #40 continua de pé, e a 2b é a próxima** |
| 126 | ✅ **REGRESSÃO LIMPA, e os dois bugs da rodada anterior VERIFICADOS COMO CONSERTADOS por medição independente:** censo de conservação **0 falhas em 240 partidas** por braço (era **35/240, 81 cartas**) e `Error` cru **0 em 240** nos dois braços (era **1/240**) | Também zerados: `AcaoInvalida` de bot e de humano, teto de 30.000 ações, e **240/240 partidas terminaram** nos dois braços. Ações checadas pelo censo: **88.945 (A)** e **90.373 (B)**. ⚠️ **"Zero em 240 partidas", NUNCA "não acontece"** — é checagem depois de CADA ação nas condições medidas (mesa de 4, humano no #0, `PATENTE_ALVO_PADRAO`, 68 Portas + 48 Tesouros, dado e embaralho reais sem semente), **não prova de impossibilidade**; cobre o caminho que a rodada anterior expôs, não todo caminho possível. 🔑 **E o zero do censo só vale por causa do GATE que rodou ANTES da medição** — um smoke em duas partes: (1) sabotagens que pulam a mão, a mochila ou cada encaixe **acusam**, e a dedup do Montante é comprovada; (2) 🔴 **contagem POSITIVA via `aplicarAcao` real**, porque *"um censo zero não distingue **nunca rodou** de **rodou e não fez nada**"* — com o Bad Stuff descartado, nada é movido nem duplicado e o censo **ficaria verde**. **Um zero de conservação sem esse gate não vale nada** |

### Sessão de 2026-08-09/10 — a fatia **2b** está CONSTRUÍDA, e a carta passou a CIRCULAR

✅ **A fatia `consumíveis (instantâneo)` construiu a QUARTA família de Itens** (branch
`feat/consumiveis-instantaneo`, **806 testes verdes**, typecheck 7/7, lint limpo).
🔴 **Gate ocular do Pedro PENDENTE — nenhum item conferido**, e o da **2a** também segue pendente.
Relato completo em
[`docs/historico/2026-08-09-consumiveis-instantaneo.md`](../historico/2026-08-09-consumiveis-instantaneo.md).

⚠️ **As #127–#134 são DESENHO construído; as #135–#140 são registros de MEDIÇÃO.** E há uma coisa
nesta leva que nenhuma sessão anterior teve: 🔴 **a #136 CORRIGE a #135 — o quarto braço do soak
inverteu a leitura que o terceiro tinha publicado.** As duas ficam escritas.

| # | Decisão | Por quê |
|---|---|---|
| 127 | 🧪 **O `instantâneo` NASCE EM CÓDIGO: quarta família de Itens, 4 cartas, e o efeito é SÓ DELTA DE STATS.** `pocao-de-cura` `{vida:+5}` · `elixir-de-forca` `{forca:+3}` · `oleo-de-precisao` `{habilidade:+2}` · `areia-nos-olhos` `{forca:−2}`. **1 cópia por jogador, 16 na mesa de 4.** ✅ **CONSTRUÍDA** | 🎚️ **Os quatro valores são DISTINTOS de propósito** (+5, +3, +2, −2): números iguais fazem dois testes passarem por **coincidência aritmética**, o que já aconteceu duas vezes nesta base. A calibragem sai dos números reais (jogador base `forca 3 / vida 10 / habilidade 6 / agilidade 5`; monstros com vida 14–28 e força 3–6; dano = `level + forca`): **+5 de vida é um golpe médio** (o Goblin dana 5, o Ogro 9); **+3 de força vale MAIS que a Espada Curta permanente (+2)**, e é o preço de ser consumível; **+2 de habilidade** leva 6→8, isto é **50% → 66,7% de acerto**; **−2 de força** leva o Ogro de 9 para 7 e **nunca é letal**. ⚠️ `efeitos` é **LISTA** e toda carta de produção tem tamanho 1 (mesmo motivo da **#120**): o laço do interpretador é percorrido **só por dublê**. ⚠️ **O Óleo de Precisão vai encostar no TETO da #107** (habilidade máx. 9): base 6 + óleo 2 = 8, e com um Diadema Élfico (+3) passaria — **quem construir a #107 tem que visitar esta carta** |
| 128 | 🎯 **O ALVO MORA NA AÇÃO, NÃO NA CARTA — o lutador OU o monstro, escolhido na hora do uso.** Emenda do Pedro sobre a primeira versão do desenho: *"posso bufar tanto a mim quanto ao monstro"*. ✅ **CONSTRUÍDA** | 🔑 **O motivo é ESTRUTURAL, não sabor:** com o alvo na ação, a carta fica com **exatamente a assinatura da `carta de combate`** do §4 (*"qualquer um, escolhendo o alvo"*) — e no bloco 5 o que muda é **quem pode jogar**, não a carta nem o interpretador. **A interoperabilidade com a interferência sai de graça.** ⚠️ **Consequência aceita: bufar o monstro ou sabotar a si mesmo é jogada LEGAL** — hoje irracional, amanhã é a mecânica inteira, e **nenhum guard a impede**. ⚠️ **A ficção fica torta** (*"Areia nos Olhos"* usada em si mesmo); aceito, porque o §16 declara toda a nomenclatura provisória. 🔴 **E ela cobra um preço no código:** a cura precisa de teto **para o monstro também**, e `EstadoCombate` só guarda `vidaInicialJogador`. **O motor NÃO foi tocado por isso** — a mesa relê a vida da carta pelo catálogo (`InfoMonstro.vida`) e passa o teto. 📊 **Custo medido do lado do exercício: `alvo: 'monstro'` só apareceu com a Areia nos Olhos** (identidade exata, 938 de 938), então *"bufar o monstro como blefe"* — o caso que o bloco 5 existe para criar — tem **ZERO exercício** no soak |
| 129 | ✅ **A CURA TEM TETO NA VIDA INICIAL DO ALVO — `min(vida + n, vidaInicial)`. FECHA A PERGUNTA 15 do §18.** ✅ **CONSTRUÍDA** | Aberta desde 2026-07-29. ⚠️ **A pergunta estava escrita num vocabulário que o motor não tem:** não existe "vida máxima" em `Combatente` — `vida` é contador que só desce, e `vidaInicialJogador` existe só como referência para passivas. **A única tradução possível em código é: a cura tem cap, ou não tem.** **O que o cap compra:** jogar a poção com a vida cheia **desperdiça**, e é isso que cria a decisão *"agora, ou aguento mais um golpe?"*; sem cap, a jogada certa é sempre *"clica assim que comprar"*. 💰 **Custo aceito e declarado ANTES de medir:** a carta com cap **circula menos**, e circulação é o critério desta fatia. 📊 **Medido: a Poção fica em ~43% dos usos das outras três** — e a causa é **política do bot** (ela só entra abaixo de `LIMIAR_DE_CURA`, 40% da vida, enquanto os buffs têm a janela do turno 0), **não a regra**. **Dial de POLÍTICA, não de carta** |
| 130 | 🔴 **PISO 1 EM TODO STAT, INCLUSIVE VIDA — e o da vida NÃO é simetria: é o que torna estruturalmente impossível um instantâneo MATAR.** ✅ **CONSTRUÍDA** | **Dois problemas diferentes com a mesma linha.** (1) **Força:** `Areia nos Olhos` em série levaria a força do Rato Gigante (3) a **1 → −1**, e um combatente com força negativa **dana negativo e CURA o alvo**. O piso 1 que existe hoje mora em `somaComPiso`, dentro de `montarCombatente` — e **este caminho não passa por lá**, porque o instantâneo mexe no `Combatente` **já montado**. (2) **Vida:** o desfecho do combate é decidido **dentro do motor**, e este caminho passa por fora dele — um alvo levado a 0 aqui ficaria *"morto"* com o combate seguindo. 🔑 **É o mesmo motivo pelo qual DANO DIRETO ficou fora da fatia (#134): o piso é o que mantém a promessa.** ➡️ **Escrito no aplicador e prendido por teste** — não pode ser descoberto em produção |
| 131 | 🎒 **O CONSUMÍVEL É USÁVEL DA MÃO E DA MOCHILA, e a mochila vira ESTOQUE — com o risco declarado por escrito ANTES de medir.** `guardarCarta` passa a aceitar as duas famílias. ✅ **CONSTRUÍDA** | Decisão do Pedro. 🔴 **O risco, escrito no spec §5.1 antes do soak:** a mochila é **isenta do limite de mão**, então estocar poção sai **de graça** e a carta pode ficar parada a partida inteira — **que é exatamente a doença que a fatia veio curar**. É a pergunta **19** do §18 entrando por uma porta nova. **Não há guard contra isso; há INSTRUMENTO.** 📊 **Medido, e o risco é MODERADO: 1,98 instantâneo por partida (12,4% da tiragem) dorme na mochila; 65,0% terminam nos baralhos** (N = 16×240, braço B). 🔑 **E a mochila virou o caminho PRINCIPAL, não o risco principal: 75,6% dos usos saem dela** — o consumível é sacado no loot, **não pode ser equipado**, o bot o guarda, e ele é queimado dali no combate seguinte. ⚠️ *"Sem a zona da mochila 75,6% dos usos não existiriam"* é **DERIVAÇÃO** (a política do bot também mudaria), não medição |
| 132 | 🚫 **USO SEM EFEITO É RECUSADO (`AcaoInvalida`), com o gêmeo APAGADO na tela (convenção #26).** ✅ **CONSTRUÍDA** | **Por que um guard e não *"deixa o jogador desperdiçar"*:** sem ele existe a jogada *"queimo a carta sem efeito"*, que **devolve carta ao cemitério sem que a mecânica tenha funcionado** — e **poluiria exatamente o número que a fatia veio medir**. 🔑 **O guard é GERAL, não *"cura com vida cheia"***: um `Areia nos Olhos` contra um monstro já no piso de força também não faz nada, e um guard escrito só para a cura deixaria esse caso queimando carta de graça. 📊 **Ele NUNCA disparou no soak, e isso é ESTRUTURAL, não sorte:** o bot pré-filtra pela mesma função (`instantaneoTemEfeito`), então nunca manda a jogada que o reducer recusaria. ➡️ **Consequência declarada: o caminho do 400 fica sem exercício no soak** — quem o cobre é teste unitário |
| 133 | 📜 **A RECEITA DE TESOUROS PASSA A SER DECLARADA: 12 equipamentos + 4 instantâneos por jogador = 16/jogador, 64 na mesa, 25% CONSUMÍVEL. A mesa vai de 116 para 132 cartas.** ✅ **CONSTRUÍDA** | 🔑 **É a #40 cobrando, e o próprio código marcou a data:** o comentário de `server/src/app.ts` dizia, desde 2026-08-02, *"no dia em que o primeiro consumível nascer, esta linha vira receita declarada"*. Até aqui Tesouros **derivava** a contagem do catálogo, porque *"não há proporção para assinar quando existe uma família só"*. 🎯 **Por que 25% e não os ≥50% da #40:** a receita-alvo do §11 põe, por jogador, **equipamento 9 · carta de combate 5 · instantâneo 4** — o instantâneo **sozinho** vale ~22% do baralho de Itens, e a outra metade do consumível é a `carta de combate`, do bloco 5. **25% é a dose FIEL ao alvo, não uma dose tímida.** ✅ **O instantâneo é a segunda família a bater o número dela** (4/jogador, exato), depois da classe |
| 134 | ✂️ **RE-ROLAR O DADO, FUGIR DO COMBATE e DANO DIRETO ficam FORA — cada um com o SEU motivo, e nenhum deles é "não deu tempo".** | **Re-rolar:** preventivo custaria estado novo em `CombateNaMesa`; reativo é **pausa nova no motor** (`DecisaoPendente`, `avancar`, UI). Fatia própria. **Fuga:** desfecho novo no motor, `fecharCombate(…, venceu: boolean, …)` deixando de ser booleano, **e o PREÇO da fuga sem desenho**. 🔴 **E ela puxa a economia para o lado CONTRÁRIO:** fugir do Ogro anula justamente a evacuação que devolveu +13,57 cartas/partida (#123) — duas alavancas opostas no mesmo soak é a **#51** outra vez. **Dano direto:** pode **matar**, e o desfecho é calculado **dentro do motor**; ou a mesa reimplanta a checagem de morte (segunda cópia da regra), ou o efeito não é letal. **Escolhido: não abrir a porta** — e é a #130 que mantém isso verdadeiro no código. 🔴 **O `motor` saiu desta fatia BYTE-IDÊNTICO**, e isso era requisito escrito, não expectativa |
| 135 | 📊 **MEDIDO: o esgotamento do baralho de Tesouros vai de 90,83% para ZERO por cento das partidas (N=240 por braço) — e o braço de controle mostra que o TAMANHO sozinho já bastava.** | **Três braços, mesma invocação, mesmo build.** **A** = 12 equipamentos (48 na mesa, o controle de hoje): **218/240 = 90,83%**. **B** = a fatia (64, 25% consumível): **0/240**. **C** = 16 equipamentos (64, zero consumível): **0/240**. 🔴 **`B ≡ C` na medida-alvo** — um baralho de 64 **puramente de equipamento** apaga o esgotamento tão bem quanto o de 64 com consumível. ⚠️ **Empilhando as três sessões (720 partidas por braço) a igualdade NÃO é exata: A 646/720 = 89,72% · B 0/720 · C 6/720** — o braço C **vive perto o bastante do degrau para cruzá-lo por acaso**, o B não cruzou. 💰 **E o DANO do esgotamento, que é o que liga isto à §11: 9,31 cartas de loot que a mesa PROMETEU e não pagou, por partida** no braço A (2.235 em 240 partidas), contra **zero** nos dois braços de 64. ✅ **A circulação existe e está medida nas medidas que NÃO saturam** (B × C, mesmo tamanho, N=240 cada): o **mínimo MÉDIO** que o baralho atinge na partida, **16,46 × 10,15** cartas; Tesouros presos com os jogadores no fim **46,85 × 52,85 (−6,00 cartas)**; cauda de *"baralho ≤10 cartas"* muito mais fina no B. ⚠️ **NÃO cite um multiplicador para a cauda** — a razão C/B replicou em **76,6× / 22,7× / 14,0×** nas três sessões: o **sinal** é robusto, o **número** varia 5,5× porque o denominador é minúsculo. **A medida estável é a diferença em pontos percentuais.** ⚠️ **O braço C tem um artefato não medido:** o catálogo tem 12 ids de equipamento e ele precisa de 16, então quatro são **segundas cópias**; a direção do efeito dessa **concentração** sobre a aderência é **INDETERMINADA**, e o gap B × C repousa sobre ela |
| 136 | 🔴 **MEDIDO, e ele CORRIGE a #135: a PROPORÇÃO sozinha também basta — 86,25% → 2,08% de esgotamento no MESMO baralho de 48, à dose de 33,3% de consumível** (braço D, N=240, sessão 3) | 🔑 **O erro que este braço expôs vale mais que o número dele, e é de RACIOCÍNIO, não de desenho.** A leitura publicada com três braços era *"`B ≡ C` nesta medida, **logo** a fatia moveu o baralho, não a economia"*. A primeira metade é **observação** e continua valendo; a segunda era **INFERÊNCIA NÃO LICENCIADA** — o braço C prova que o tamanho **BASTA** (suficiência), e a frase afirmava que **só** o tamanho agiu (exclusividade). **O desenho A/B/C nunca teve um braço que testasse a proporção sozinha**, então essa metade vinha do **silêncio**. ➡️ **Num sistema SOBREDETERMINADO, nenhuma atribuição a uma alavanca é licenciada.** 📊 **D × A é o contraste mais limpo do desenho inteiro** — mesmo tamanho (48), mesmo número de ids (12), 1 cópia cada, **uma única variável**: 4 ids de equipamento trocados por 4 de instantâneo. Ações com os montes secos **12,29% → 0,046%**; cartas de loot não pagas **8,59/partida → 0,004** (uma carta em 240 partidas). ⚠️ **E o viés do desenho corre CONTRA o D** (ele carrega 50% de exclusivos contra 33% do A, e exclusivo é candidato a carta morta): **ele produziu esse efeito sabotado**. 🔴 **A dose de 33,3% NÃO é a de produção (25%), e nenhum braço mediu 25% num baralho de 48** — quem citar `86,25% → 2,08%` sem a dose autoriza uma decisão de baralho que ninguém mediu. ➡️ **A leitura que fecha as duas: a proporção move o DEGRAU, o tamanho compra a MARGEM** (D × B: mínimo médio do baralho **6,65 × 16,48**; cauda ≤10 **13,15% × 0,397%** — o baralho menor vive muito mais perto da borda **sem colapsar**) |
| 137 | 🔑 **MEDIDO, e é o achado mais transferível da fatia: ADERÊNCIA NÃO É PROPRIEDADE DA FAMÍLIA NEM DA MESA — é função da ESCASSEZ RELATIVA daquela família. ➡️ Dobrar a dose NÃO dobra a circulação** | **A escada, toda re-fonteada na MESMA sessão** (N = carta × partida): equipamento **83,41%** com 64 na mesa · **85,56%** com 48 · **95,73%** com 32; instantâneo **35,44%** com 16-em-64 · **59,27%** com 16-em-48. **Monotônica nas duas famílias: menos oferta ⇒ mais aderência.** 🔑 **Ela nasceu de uma PREVISÃO REGISTRADA QUE ERROU**, e é por isso que vale: a previsão do braço D foi escrita em disco **antes** de o braço existir, tratou aderência como **constante por família** (34,95%, o valor que estava na mão) e cravou *"5,6–8 instantâneos presos"*; o medido foi **9,48 (59,27%)**. **A decisão prevista acertou (esgotamento < 20%, deu 2,08%); o MECANISMO errou.** ⚠️ **A FORMA da curva entre os pontos NÃO foi medida** — são três pontos no equipamento e **dois** no instantâneo, e é dela que sairia *"quanto do ganho de dobrar a dose é comido pela aderência"*. 📌 **Consequência direta para a #40:** um dial de dose **não** é linear, e girar 25% → 50% não entrega o dobro de circulação |
| 138 | ✅ **CONTAGEM POSITIVA, e ela vem ANTES de qualquer conclusão: 3.260 usos de instantâneo no braço B — 13,58 por partida, 3,40 por jogador, 240/240 partidas com ao menos um uso.** | 🔴 **Não é zelo, é a lição da 2a (§15 das lições): *censo de conservação zero NÃO prova que a feature rodou*** — ele não distingue *"nunca rodou"* de *"rodou e não fez nada"*. **Se o braço B medisse zero uso, todo o resto da medição não significaria nada.** ✅ **As ações `usarInstantaneo` aceitas pelo reducer batem EXATO com os eventos emitidos** (3.260 = 3.260). **Por carta** (N=240 partidas): Elixir 958 · Óleo 956 · Areia 938 · **Poção 408** (a janela dela é outra — ver #129). **Por zona: mochila 2.466 (75,6%) · mão 794 (24,4%)**. **Por alvo: lutador 2.322 · monstro 938** — 🔑 **e `monstro = 938` é EXATAMENTE a contagem da Areia nos Olhos, identidade ESTRUTURAL** (o bot só manda ao monstro o efeito de soma negativa, e a Areia é o único). 🔴 **Braços A e C: ZERO usos, ESTRUTURAL e não empírico** — as composições deles não têm uma única carta de instantâneo, então a ação não tem carta para apontar. **Não é medição de que "ninguém usou"** |
| 139 | 📊 **O GRADIENTE DE ASSENTO DEIXA DE SER RUÍDO: monotônico nos DEZ braços independentes, 860 · 654 · 513 · 373 em 2.400 partidas (35,8% · 27,3% · 21,4% · 15,5%). EFEITO REAL, CAUSA NÃO ISOLADA** — a pergunta **17** do §18 muda de estado | **O que mudou não foi uma medição a mais, foi o FORMATO:** dez braços, três sessões, e a ordem `#0 > #1 > #2 > #3` **sem uma única inversão**. A dispersão do **mesmo** assento entre braços fica em ~5–9pp contra um vão **#0−#3 de 15–25pp em cada braço** (⚠️ **derivação sobre as contagens publicadas**, não medida do harness). 🔴 **NENHUMA causa é atribuída, e o confundidor está NOMEADO e não isolado: o assento #0 age primeiro**, e os quatro rodam a **mesma** `escolherAcao` (#51). Nenhum braço foi desenhado para separar iniciativa de qualquer outra coisa. 🔴 **E uma distinção que não pode ser borrada:** *usos de instantâneo* por assento são **RUIDOSOS** (3 das 4 observações quebram a monotonicidade); ***vitórias* por assento NÃO são** (10 de 10). **Quem confundir as duas contamina esta pergunta.** ⚠️ **A linha a escrever continua sendo *"o último assento vence menos"*, NUNCA a escada** |
| 140 | ✅ **REGRESSÃO LIMPA: 0 falhas de censo de conservação em 369.061 ações (960/960 partidas terminadas), 0 `AcaoInvalida`, 0 `Error` cru, 0 cartas distintas perdidas** | **Censo id-a-id depois de CADA ação, nos quatro braços da sessão 3.** ✅ **E o zero só vale por causa do GATE que rodou ANTES:** num estado intocado o censo dá `[]`; com **uma carta apagada da mão** ele acusa exatamente ela; e com o **instantâneo na mochila** ele dá limpo, mas **sabotado sem a mochila** ele acusa — ou seja, **a zona nova desta fatia está DENTRO do censo**, que é exatamente o que um script esqueceu uma vez (`emJogo.raca`). ⚠️ **"Zero em 240 partidas por braço", NUNCA "não acontece"** — é checagem nas condições medidas (mesa de 4, humano no #0, `PATENTE_ALVO_PADRAO` importado, mão 4+4, **68 Portas + 64 Tesouros**, dado e embaralho reais sem semente), não prova de impossibilidade. ✅ **O harness replicou a fatia anterior dentro de 1pp** (braço A: 90,83% contra os 91,7% que a 2a mediu na mesma mesa de 48) — é o que valida um instrumento **reescrito do zero pela sétima fatia seguida** |
